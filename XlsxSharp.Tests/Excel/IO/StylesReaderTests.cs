...
using XlsxSharp.IO;
using StylesReader = XlsxSharp.Excel.IO.StylesReader;
...
internal class StylesReaderTests
{
    ...
    {
        using MemoryStream stream = new(XLHelper.NoBomUTF8.GetBytes(xml));
        using XmlTreeReader xmlTreeReader = new(stream, XmlToEnumMapper.Instance, true);
        XLWorkbookStyles styles = new();
        StylesReader reader = new(xmlTreeReader, styles);
        reader.Load();
        assert(styles);
    }
}
