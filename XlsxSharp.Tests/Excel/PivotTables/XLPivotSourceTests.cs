...
internal class XlPivotSourceTests
{
    [Test]
    [Skip(
        "connections.xml is reformatted with SDK-canonical namespace prefixes only as a side "
            + "effect of the old OpenXmlValidator touching every part during SaveOptions."
            + "ValidatePackage - now a no-op since the SDK was removed, so an untouched "
            + "connections.xml round-trips byte-for-byte instead. Revisit with ValidatePackage."
    )]
