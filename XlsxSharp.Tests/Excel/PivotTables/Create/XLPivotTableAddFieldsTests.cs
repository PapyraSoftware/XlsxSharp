...
namespace XlsxSharp.Tests.Excel.PivotTables.Create;
...
internal class XLPivotTableAddFieldsTests
{
    ...
    public void Add_one_column_without_value()
    {
        TestHelper.CreateAndCompare(
            wb =>
            {
                IXLPivotTable pt = CreatePivotTableFor2X2(wb);
    ...
    {
        IXLRange range = wb.AddWorksheet()
            .FirstCell()
            .InsertData(new object[] { ("A", "B"), (1, 2) });

        IXLWorksheet ws = wb.AddWorksheet().SetTabActive();
        IXLPivotTable pt = ws.PivotTables.Add("Test", ws.FirstCell(), range);
        return pt;
    }
}
