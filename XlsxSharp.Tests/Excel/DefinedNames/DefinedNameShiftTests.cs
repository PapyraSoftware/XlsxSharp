...
internal class DefinedNameShiftTests
{
    ...
    )
    {
        ...
        IXLWorksheet shiftSheet = wb.AddWorksheet("Shift");
        IXLWorksheet otherSheet = wb.AddWorksheet("Other");

        IXLDefinedName workbookName = wb.DefinedNames.Add("WorkbookName", "Shift!$C$3:$D$5");
        IXLDefinedName shiftName = shiftSheet.DefinedNames.Add("ShiftName", "Shift!$C$3:$D$5");
        IXLDefinedName otherName = otherSheet.DefinedNames.Add("OtherName", "Other!$C$3:$D$5");
    ...
    public void Shift_references_inside_formulas()
    {
        ...
        using XLWorkbook wb = new();
        IXLWorksheet lookupSheet = wb.AddWorksheet("Lookup");
        IXLDefinedName name = wb.DefinedNames.Add(
            "Name",
            "OFFSET(Lookup!$G$2,0,0,COUNTA(Lookup!$G:$G)-1,1)"
        );
    ...
    public static IEnumerable<object[]> GetShiftTestCases
    {
        get
        {
            ...
            yield return Test(
                ws => ws.Range("C2:D4").Delete(XLShiftDeletedCells.ShiftCellsUp),
                "Shift!$C$2:$D$2"
            );

            // Delete area and shift cells left. Also partially shrinks reference to 1-column from 2-column original.
            yield return Test(
                ws => ws.Range("B2:C7").Delete(XLShiftDeletedCells.ShiftCellsLeft),
                "Shift!$B$3:$B$5"
            );

            // When the whole area is deleted, the defined name is a #REF! error.
            yield return Test(
                ws => ws.Range("A1:E8").Delete(XLShiftDeletedCells.ShiftCellsLeft),
                "#REF!"
            );
            ...
            yield return Test(
                ws => ws.Range("C2").Delete(XLShiftDeletedCells.ShiftCellsUp),
                "Shift!$C$3:$D$5"
            );
