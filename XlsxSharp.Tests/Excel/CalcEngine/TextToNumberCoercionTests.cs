...
public class TextToNumberCoercionTests
{
    ...
    [Test]
    public void TimeSpanMaximumResolutionIsOneMs()
    {
        double firstValue = (double)XLWorkbook.EvaluateExpr("\"0:0:0.0015\" * 1");
        double secondValue = (double)XLWorkbook.EvaluateExpr("\"0:0:0.0024\" * 1");
    ...
    [Arguments(
        " - 100 % ",
        -1,
        Skip = ".NET parser doesn't allow whitespace between sign and number."
    )]
    ...
    [Arguments(
        " 1-jan-2022",
        null,
        Skip = ".NET parser doesn't respect the whitespace styles of a date during parsing."
    )] // Can't have whitespaces at the start
    ...
    [Arguments(
        "1-marc",
        44621,
        Skip = ".NET parser recognizes only abbreviation or full name of a month."
    )]
    ...
    public void DateFormat16(string text, double? expectedValue) // Format 16 'd-mmm'
    {
        if (expectedValue is not null)
        {
            DateTime date = DateTime.FromOADate(expectedValue.Value);
    ...
    public void DateFormat16UsesCulture(string text, double? expectedValue) // Format 16 'd-mmm'
    {
        expectedValue +=
            new DateTime(DateTime.Now.Year, 1, 1).ToOADate() - new DateTime(2000, 1, 1).ToOADate();
        AssertCoercion(text, expectedValue);
    }
    ...
    [Test] // interpreted as 2022-01-02
    [Arguments(
        "jan-02",
        44563,
        Skip = ".NET misses culture, en interprets it as MMM-dd, but czech as MMM-yy, so the MMM-dd is the extra culture for en."
    )] // interpreted as 2022-01-02
    ...
    [Arguments(
        " aug-55",
        null,
        Skip = ".NET allow whitespaces even without specified DateTimeStyle.AllowLeadingWhite"
    )] // starting spaces not allowed
    ...
    [Arguments(
        "11/30/2022 24:59",
        44896.04097,
        Skip = "Excel can have out of range parts, but .NET parsers can't."
    )]
    [Arguments("11/30/2022 24:60", null)] // Both parts are out of range
    [Arguments(
        "11/30/2022 23:160",
        44896.06944,
        Skip = "Excel can have one of of range part, but .NET parser can't."
    )]
    [Arguments(
        "11/30/2022 9999:59",
        45311.66597,
        Skip = "Excel parser accepts numbers over limit for hours."
    )]
    [Arguments("11/30/2022 10000:59", null)] // Hours can't be over 9999
    [Arguments(
        "aug 10, 2022 14:10",
        44783.590277777781d,
        Skip = "Excel specific parsing of months accepts anything from three letters up to full name, but such pattern is not in any en-US DateTimeFormat pattern."
    )]
    ...
    [Arguments(
        "1,00",
        null,
        Skip = ".NET parse methods ignores thousands separator, but excel enforces them."
    )]
    ...
    [Arguments(
        "  (   1,000.54  )  ",
        -1000.54,
        Skip = "Excel can parse spaces within braces, but .NET parse method can't."
    )]
    ...
    [Arguments(
        "1$",
        null,
        Skip = ".NET parser allows currency symbol at the start or end, but Excel requires correct placement."
    )]
    [Arguments("($1)", -1)]
    [Arguments("-($1)", null)]
    ...
    [Arguments(
        "Kč 1",
        null,
        Skip = "Excel requires correct placement of currency symbol, while .NET parser accepts any position."
    )] // incorrect placement
    ...
    [Arguments(
        "12:0:18 odp",
        0.50020833333333337d,
        Skip = "Excel can parse even partial PM designator, but .NET requires a full PM designator including the dot at the end."
    )]
    [Arguments(
        "12:0:18 PM.",
        0.50020833333333337d,
        Skip = "Excel allows PM designator with a dot at the end."
    )]
    ...
    private static void AssertCoercion(string text, double? expectedValue, double tolerance = 0)
    {
        ...
        XLCellValue parsedValue = wb.Evaluate($"\"{text}\"*1");
        if (expectedValue is null)
        {
            ClassicAssert.AreEqual(XLError.IncompatibleValue, parsedValue);
        }
        else
        {
            ClassicAssert.AreEqual(expectedValue.Value, (double)parsedValue, tolerance);
        }
    }
}
