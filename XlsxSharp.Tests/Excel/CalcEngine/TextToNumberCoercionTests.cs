...
public class TextToNumberCoercionTests
{
    ...
    [TestCase(
        " 1-jan-2022",
        null,
        Ignore = ".NET parser doesn't respect the whitespace styles of a date during parsing."
    ...
    [TestCase(
        "1-marc",
        44621,
        Ignore = ".NET parser recognizes only abbreviation or full name of a month."
    )]
    ...
    [TestCase(
        "jan-02",
        44563,
        Ignore = ".NET misses culture, en interprets it as MMM-dd, but czech as MMM-yy, so the MMM-dd is the extra culture for en."
    ...
    [TestCase(
        " aug-55",
        null,
        Ignore = ".NET allow whitespaces even without specified DateTimeStyle.AllowLeadingWhite"
    ...
    [TestCase(
        "11/30/2022 24:59",
        44896.04097,
        Ignore = "Excel can have out of range parts, but .NET parsers can't."
    )]
    [TestCase("11/30/2022 24:60", null)] // Both parts are out of range
    [TestCase(
        "11/30/2022 23:160",
        44896.06944,
        Ignore = "Excel can have one of of range part, but .NET parser can't."
    )]
    [TestCase(
        "11/30/2022 9999:59",
        45311.66597,
        Ignore = "Excel parser accepts numbers over limit for hours."
    )]
    ...
    [TestCase(
        "1,00",
        null,
        Ignore = ".NET parse methods ignores thousands separator, but excel enforces them."
    )]
    ...
    [TestCase(
        "1$",
        null,
        Ignore = ".NET parser allows currency symbol at the start or end, but Excel requires correct placement."
    )]
    [TestCase("($1)", -1)]
    [TestCase("-($1)", null)]
    ...
    [TestCase(
        "Kč 1",
        null,
        Ignore = "Excel requires correct placement of currency symbol, while .NET parser accepts any position."
    )] // incorrect placement
    ...
    [TestCase(
        ...
        Ignore = "Excel can parse even partial PM designator, but .NET requires a full PM designator including the dot at the end."
    )]
    [TestCase(
        "12:0:18 PM.",
        0.50020833333333337d,
        Ignore = "Excel allows PM designator with a dot at the end."
    )]
