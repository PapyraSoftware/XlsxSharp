...
internal class StructuredReferenceTests
{
    ...
    )
    {
        ...
        IXLTable table = Add4X3Table(ws, "E7");
    ...
    public void Structured_reference_uses_cells_from_table_worksheet()
    {
        ...
        // https://github.com/XlsxSharp/XlsxSharp/issues/2871
        using XLWorkbook wb = new();
        IXLWorksheet tableSheet = wb.AddWorksheet("TableSheet");
        tableSheet
            .Cell("B3")
            .InsertTable(
                [new { Name = "Cake", Price = 7 }, new { Name = "Pie", Price = 9 }],
                "Pastries"
            );
        IXLWorksheet formulaSheet = wb.AddWorksheet();
    ...
    private static IXLTable Add4X3Table(IXLWorksheet ws, string origin)
    {
        ...
        IXLTable table = ws.Cell(origin).InsertTable(dt, "TableName");
        table.SetShowTotalsRow(true);
        return table;
    }

    private static void AssertRange(
        string structureReference,
        string expectedArea,
        IXLWorksheet ws,
        string? formulaAddress = null
    )
    {
        if (expectedArea == "#REF!")
        {
