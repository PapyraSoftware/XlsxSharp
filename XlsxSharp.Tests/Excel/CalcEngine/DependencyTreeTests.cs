...
internal class DependencyTreeTests
{
    ...
    {
        FormulaDependencies dependencies = GetDependencies(formula);
    ...
    public void Name_range_that_is_reference_is_propagated_to_formula()
    {
        ...
        CollectionAssert.AreEquivalent(
            new SheetArea[] { new("Sheet", Area.Parse("B3:D7")) },
            dependencies.Areas
        );
        CollectionAssert.AreEquivalent(new[] { new XLName("name") }, dependencies.Names);
    }
    ...
    public void Name_range_can_used_another_name_range()
    {
        ...
        CollectionAssert.AreEquivalent(
            new SheetArea[] { new("Sheet", Area.Parse("D7")), new("Sheet", Area.Parse("B1")) },
            dependencies.Areas
        );
        CollectionAssert.AreEquivalent(
            new[] { new XLName("outer"), new XLName("inner") },
            dependencies.Names
        );
    }
    ...
    public void Name_range_can_be_sheet_scoped_even_without_specified_sheet()
    {
        ...
        CollectionAssert.AreEquivalent(
            new SheetArea[] { new("Sheet", Area.Parse("A1")) },
            dependencies.Areas
        );
        CollectionAssert.AreEquivalent(new[] { new XLName("name") }, dependencies.Names);
    }
    ...
    {
        using XLWorkbook wb = new();
        IXLWorksheet renamedSheet = wb.AddWorksheet("Original");
        IXLWorksheet unchangedSheet = wb.AddWorksheet("Unchanged");
    ...
    private static XLCellFormula AddFormula(
        DependencyTree tree,
        IXLWorksheet sheet,
        string address,
        string formula
    )
    {
        ...
        XLCell cell = (XLCell)sheet.Cell(address);
        cell.Formula = XLCellFormula.NormalA1(formula);
        SheetArea cellArea = new(sheet.Name, new Area(cell.Point, cell.Point));
    ...
    {
        SheetArea area = new(sheet.Name, Area.Parse(range));
        tree.MarkDirty(area);
    }
    ...
    private static void AssertDirtyFlag(
        ...
        IXLWorksheet sheet,
        params string[] dirtyRanges
    )
    {
        XLWorksheet ws = (XLWorksheet)sheet;
        foreach (string dirtyRange in dirtyRanges)
        {
    ...
    private static FormulaDependencies GetDependencies(
        string formula,
        string formulaAddress = "A1",
        Action<XLWorkbook> init = null
    )
    {
        using XLWorkbook wb = new();
        ...
        init?.Invoke(wb);
        DependencyTree tree = new();
        IXLCell cell = ws.Cell(formulaAddress);
        cell.SetFormulaA1(formula);

        XLCellFormula cellFormula = ((XLCell)cell).Formula;
        FormulaDependencies dependencies = tree.AddFormula(
            new SheetArea(ws.Name, cellFormula.Range),
            cellFormula,
            wb
        );
        return dependencies;
    ...
    public static IEnumerable<object[]> AreaDependenciesTestCases
    {
        get
        {
            ...
            yield return ["A1", new[] { new SheetArea("Sheet", Area.Parse("A1")) }];
            ...
            yield return ["4+A4%", new[] { new SheetArea("Sheet", Area.Parse("A4")) }];

            // Union operation propagates references
            yield return
            [
                "(A1:B2,C1:D2):E3",
                new[] { new SheetArea("Sheet", Area.Parse("A1:E3")) },
            ];
            ...
            [
                ...
                "A1:C4:D2",
                new[] { new SheetArea("Sheet", Area.Parse("A1:D4")) },
            ];
            ...
            yield return ["INDEX({1},1,1):D2", new[] { new SheetArea("Sheet", Area.Parse("D2")) }];
    ...
    public static IEnumerable<object[]> NameDependenciesTestCases
    {
        get
        {
            yield return ["WorkbookName  + 5", new[] { new XLName("WorkbookName") }];

            yield return ["Sheet!Name", new[] { new XLName("Sheet", "Name") }];
        }
    }
}
