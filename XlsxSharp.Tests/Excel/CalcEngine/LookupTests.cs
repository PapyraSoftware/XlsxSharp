...
public class LookupTests
{
    ...
    public void Hlookup()
    {
        ...
        Assert.AreEqual(XLError.NoValueAvailable, this.ws.Evaluate(@"HLOOKUP("""",A2:E2,1,FALSE)"));
        Assert.AreEqual(XLError.NoValueAvailable, this.ws.Evaluate(@"HLOOKUP(50,B2:E3,1,FALSE)"));

        // Value in approximate search that is lower than first element
        Assert.AreEqual(XLError.NoValueAvailable, this.ws.Evaluate(@"HLOOKUP(-10,B2:E3,2,TRUE)"));
    }
    ...
    public void Hyperlink()
    {
        ...
        Assert.AreEqual("http://github.com/XlsxSharp/XlsxSharp", cell.Value);
    ...
    public void VlookupUnexpectedArguments()
    {
        ...
        // Range can only be array or a reference. If other type, it returns the error #N/A
        Assert.AreEqual(XLError.NoValueAvailable, this.ws.Evaluate("=VLOOKUP(1,1,1)"));
        Assert.AreEqual(XLError.NoValueAvailable, this.ws.Evaluate("=VLOOKUP(1,TRUE,1)"));

        // If range is a non-contiguous range, #N/A
        Assert.AreEqual(XLError.NoValueAvailable, this.ws.Evaluate("=VLOOKUP(1,(B2:I5,B6:I10),1)"));

        // The column index must be at most the same as width of the range. It is 9 here, but range is 8 cell wide.
        Assert.AreEqual(XLError.CellReference, this.ws.Evaluate("=VLOOKUP(20,B2:I71,9,FALSE)"));
        // The column index must be at least 1. It is 0 here.
        Assert.AreEqual(XLError.IncompatibleValue, this.ws.Evaluate("=VLOOKUP(20,B2:I71,0,FALSE)"));
    }
    ...
    public void VlookupColumnIndexParameterUsesValueSemantic()
    {
        ...
        Assert.AreEqual(@"Jardine", this.ws.Evaluate("=VLOOKUP(3,B2:I71,\"2 5/2\")"));
    }
