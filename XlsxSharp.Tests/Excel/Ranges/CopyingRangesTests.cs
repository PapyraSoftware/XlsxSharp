...
public class CopyingRangesTests
{
    ...
    public void CopyingConditionalFormatsDifferentWorksheets()
    {
        ...
        IXLConditionalFormat format = ws1.Range("A1:J2").AddConditionalFormat();

        string address = format.Ranges.First().FirstCell().CellRight(4).Address.ToStringRelative();

        format.WhenEquals("=" + address).Fill.SetBackgroundColor(XLColor.Blue);
    ...
    public void CopyCellFormatWithSomeComponentsAlreadyUsedInTargetWorkbook()
    {
        ...
        TestHelper.CreateSaveLoadAssert(
            (_, ws) =>
            {
                using XLWorkbook otherWb = new();
                IXLWorksheet otherWs = otherWb.AddWorksheet();
    ...
    private static void FillRow(IXLRow row1)
    {
        ...
        row1.Cell(2)
            ...
            .WhenEquals("=" + row1.FirstCell().CellRight(6).Address.ToStringRelative())
