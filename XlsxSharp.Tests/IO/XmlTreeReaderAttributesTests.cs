...
internal class XmlTreeReaderAttributesTests
{
    ...
    {
        DateTimeOffset? expectedValue = expectedString is not null
            ? DateTimeOffset.Parse(expectedString)
            : null;
        using XmlTreeReader reader = CreateReader(xmlText);
    ...
    {
        XmlToEnumMapper mapper = new XmlToEnumMapper.Builder()
            .Add(
                new BiDictionary<string, BindingFlags>
                {
    ...
    private static XmlTreeReader CreateReader(string attributeValue, XmlToEnumMapper mapper = null)
    {
        ...
        XmlTreeReader reader = new(stream, mapper, false);
