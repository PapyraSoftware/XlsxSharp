...
public static class PackageHelper
{
    ...
    public static object ReadXmlPart(Package package, Uri uri, XmlSerializer serializer)
    {
        if (!package.PartExists(uri))
        {
            throw new ApplicationException(
                string.Format("Package part '{0}' doesn't exists!", uri.OriginalString)
            );
        }
    ...
    public static void CopyPart(Uri uri, Package source, Package dest, bool overwrite)
    {
        ...
        PackagePart destPart = dest.CreatePart(
            uri,
    ...
    public static void WritePart<T>(
        Package package,
        PackagePartDescriptor descriptor,
        T content,
    ...
    )
    {
        ...
        Dictionary<Uri, PartPair> pairs = new();
        ...
        foreach (PartPair pair in pairs.Values)
        {
            ...
            PackagePart leftPart = left.GetPart(pair.Uri);
            PackagePart rightPart = right.GetPart(pair.Uri);
            using (
                Stream leftPackagePartStream = leftPart.GetStream(FileMode.Open, FileAccess.Read)
            )
            using (
                Stream rightPackagePartStream = rightPart.GetStream(FileMode.Open, FileAccess.Read)
            )
            using (MemoryStream leftMemoryStream = new())
            using (MemoryStream rightMemoryStream = new())
            {
                ...
                bool stripColumnWidthsFromSheet =
                    TestHelper.StripColumnWidths
                    && leftPart.ContentType
                        == @"application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"
                    && rightPart.ContentType
                        == @"application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml";

                (string ContentType, MemoryStream Stream) tuple1 = (
                    leftPart.ContentType,
                    Stream: leftMemoryStream
                );
                (string ContentType, MemoryStream Stream) tuple2 = (
                    rightPart.ContentType,
                    Stream: rightMemoryStream
                );
        ...
        EXIT:
        List<PartPair> sortedPairs = [.. pairs.Values];
        sortedPairs.Sort(
            (one, other) => one.Uri.OriginalString.CompareTo(other.Uri.OriginalString)
        );
