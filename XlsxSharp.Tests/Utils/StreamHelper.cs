...
public static class StreamHelper
{
        ...
        (List<(string PartSubstring, XName NodeName, XName AttrName)>)
            [
                ...
                (
                    ...
                    XName.Get("sst", @"http://schemas.openxmlformats.org/spreadsheetml/2006/main"),
                    XName.Get("count")
                ),
                (
                    "/xl/sharedStrings.xml",
                    XName.Get("sst", @"http://schemas.openxmlformats.org/spreadsheetml/2006/main"),
                    XName.Get("uniqueCount")
                ),
            ];
    ...
    public static bool Compare(
        (string ContentType, Stream Stream) left,
        (string ContentType, Stream Stream) right,
        Uri partUri,
        bool stripColumnWidths
    )
    {
        #region Check
        ...
        string leftString = new StreamReader(left.Stream).ReadToEnd();
        string rightString = new StreamReader(right.Stream).ReadToEnd();

        bool isXmlContent = left.ContentType.EndsWith("+xml");
        if (!isXmlContent)
        {
            return leftString == rightString;
        }

        XDocument leftXml = XDocument.Parse(leftString);
        RemoveIgnoredParts(leftXml, partUri, stripColumnWidths);
        Normalize(leftXml);
        XDocument rightXml = XDocument.Parse(rightString);
    ...
    private static void Normalize(XDocument document)
    {
        ...
        foreach (
            XElement emptyElement in document
                .Descendants()
                .Where(e => !e.IsEmpty && !e.Nodes().Any())
        )
        {
