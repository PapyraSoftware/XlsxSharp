...
internal static class ColorStringParser
{
    internal static Color ParseFromHtml(string argbColor)
    {
        ...
        if (argb.Length == 3)
        {
            int r = ReadHex(argb, 0, 1);
            int g = ReadHex(argb, 1, 1);
            int b = ReadHex(argb, 2, 1);
    ...
    internal static Color ParseFromArgb(ReadOnlySpan<char> argb)
    {
        ...
        int index = 0;
        while (index < argb.Length && TryGetHex(argb[index], out uint hexDigit))
        {
    ...
    {
        if (!TryParseRgb6(rgbColor.AsSpan(), out Color color))
        {
    ...
    internal static bool TryParseRgb6(ReadOnlySpan<char> rgbColor, out Color color)
    {
        ...
        if (
            !TryReadHex(rgbColor, 0, 2, out int red)
            || !TryReadHex(rgbColor, 2, 2, out int green)
            || !TryReadHex(rgbColor, 4, 2, out int blue)
        )
        {
            color = default;
            return false;
    ...
    {
        if (!TryReadHex(text, start, length, out int hexValue))
        {
    ...
    private static bool TryReadHex(ReadOnlySpan<char> text, int start, int length, out int result)
    {
        ...
        {
            if (!TryGetHex(text[i], out uint hexDigit))
            {
                result = 0;
                return false;
            }
