...
internal class PivotTableDefinitionPartWriter2
{
    ...
    )
    {
        ...
        (int Width, int Height) filterArea = pt.Filters.GetSize();
        ...
        foreach (XLPivotTableField pf in pt.PivotFields)
        {
            ...
            {
                string axisAttr = GetAxisAttr(pf.Axis.Value);
            ...
            xml.WriteAttributeDefault(
                ...
                pf.MultipleItemSelectionAllowed,
                false
            );
            ...
            {
                string sortTypeAttr = pf.SortType switch
                {
            ...
            xml.WriteAttributeDefault(
                "defaultSubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.Automatic),
                true
            );
            xml.WriteAttributeDefault(
                "sumSubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.Sum),
                false
            );
            xml.WriteAttributeDefault(
                "countASubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.Count),
                false
            );
            xml.WriteAttributeDefault(
                "avgSubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.Average),
                false
            );
            xml.WriteAttributeDefault(
                "maxSubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.Maximum),
                false
            );
            xml.WriteAttributeDefault(
                "minSubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.Minimum),
                false
            );
            xml.WriteAttributeDefault(
                "productSubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.Product),
                false
            );
            xml.WriteAttributeDefault(
                "countSubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.CountNumbers),
                false
            );
            xml.WriteAttributeDefault(
                "stdDevSubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.StandardDeviation),
                false
            );
            xml.WriteAttributeDefault(
                "stdDevPSubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.PopulationStandardDeviation),
                false
            );
            xml.WriteAttributeDefault(
                "varSubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.Variance),
                false
            );
            xml.WriteAttributeDefault(
                "varPSubtotal",
                pf.Subtotals.Contains(XLSubtotalFunction.PopulationVariance),
                false
            );
            ...
            if (pf.Items.Count > 0)
            {
                ...
                foreach (XLPivotFieldItem pfItem in pf.Items)
                {
                    ...
                    {
                        string itemTypeAttr = GetItemTypeAttr(pfItem.ItemType);
        ...
        IReadOnlyList<XLPivotPageField> filterFields = pt.Filters.Fields;
        if (filterFields.Count > 0)
        {
            xml.WriteStartElement("pageFields", Main2006SsNs);
            xml.WriteAttribute("count", filterFields.Count);
            foreach (XLPivotPageField filterField in filterFields)
            {
        ...
        if (pt.DataFields.Count > 0)
        {
            ...
            foreach (XLPivotDataField dataField in pt.DataFields)
            {
                ...
                {
                    string subtotalAttr = dataField.Subtotal switch
                    {
                ...
                {
                    string showDataAsAttr = dataField.ShowDataAsFormat switch
                    {
                ...
                xml.WriteAttributeOptional(
                    "numFmtId",
                    context.GetNumberFormatId(dataField.NumberFormatValue)
                );
        ...
        if (pt.Formats.Count > 0)
        {
            ...
            foreach (XLPivotFormat format in pt.Formats)
            {
                ...
                {
                    string actionAttr = format.Action switch
                    {
                ...
                {
                    uint dxfId = context.GetDxfId(format.FormatValue);
                    xml.WriteAttribute("dxfId", dxfId);
                }

                XLPivotArea pivotArea = format.PivotArea;
        ...
        if (pt.ConditionalFormats.Count > 0)
        {
            ...
            foreach (XLPivotConditionalFormat conditionalFormat in pt.ConditionalFormats)
            {
                xml.WriteStartElement("conditionalFormat", Main2006SsNs);
                if (conditionalFormat.Scope != XLPivotCfScope.SelectedCells)
                {
                    string scopeAttr = conditionalFormat.Scope switch
                    {
                ...
                {
                    string typeAttr = conditionalFormat.Type switch
                    {
                ...
                foreach (XLPivotArea pivotArea in conditionalFormat.Areas)
                {
                    WritePivotArea(xml, pivotArea);
                }
    ...
    private static void WriteAxis(
        XmlWriter xml,
        XLPivotTableAxis axis,
        string fieldsElement,
        string itemsElement
    )
    {
        if (axis.Fields.Count > 0)
        {
            ...
            foreach (FieldIndex axisField in axis.Fields)
            {
        ...
        if (axis.Items.Count > 0)
        {
            ...
            foreach (XLPivotFieldAxisItem axisItem in axis.Items)
            {
                xml.WriteStartElement("i", Main2006SsNs);
                if (axisItem.ItemType != XLPivotItemType.Data)
                {
                    string itemTypeAttr = GetItemTypeAttr(axisItem.ItemType);
                    xml.WriteAttribute("t", itemTypeAttr);
                }

                // 'r' attribute means repeat data from previous axis item.
                int r = 0;
                int maxPrefixLen = Math.Min(previous.Count, axisItem.FieldItem.Count);
                ...
                foreach (int fieldItem in axisItem.FieldItem.Skip(r))
                {
    ...
    private static void WritePivotArea(XmlWriter xml, XLPivotArea pivotArea)
    {
        ...
        {
            string typeAttr = pivotArea.Type switch
            {
        ...
        if (pivotArea.References.Count > 0)
        {
            ...
            foreach (XLPivotReference reference in pivotArea.References)
            {
                ...
                HashSet<XLSubtotalFunction> subtotals = reference.Subtotals;
                xml.WriteAttributeDefault(
                    "defaultSubtotal",
                    subtotals.Contains(XLSubtotalFunction.Automatic),
                    false
                );
                xml.WriteAttributeDefault(
                    "sumSubtotal",
                    subtotals.Contains(XLSubtotalFunction.Sum),
                    false
                );
                xml.WriteAttributeDefault(
                    "countASubtotal",
                    subtotals.Contains(XLSubtotalFunction.Count),
                    false
                );
                xml.WriteAttributeDefault(
                    "avgSubtotal",
                    subtotals.Contains(XLSubtotalFunction.Average),
                    false
                );
                xml.WriteAttributeDefault(
                    "maxSubtotal",
                    subtotals.Contains(XLSubtotalFunction.Maximum),
                    false
                );
                xml.WriteAttributeDefault(
                    "minSubtotal",
                    subtotals.Contains(XLSubtotalFunction.Minimum),
                    false
                );
                xml.WriteAttributeDefault(
                    "productSubtotal",
                    subtotals.Contains(XLSubtotalFunction.Product),
                    false
                );
                xml.WriteAttributeDefault(
                    "countSubtotal",
                    subtotals.Contains(XLSubtotalFunction.CountNumbers),
                    false
                );
                xml.WriteAttributeDefault(
                    "stdDevSubtotal",
                    subtotals.Contains(XLSubtotalFunction.StandardDeviation),
                    false
                );
                xml.WriteAttributeDefault(
                    "stdDevPSubtotal",
                    subtotals.Contains(XLSubtotalFunction.PopulationStandardDeviation),
                    false
                );
                xml.WriteAttributeDefault(
                    "varSubtotal",
                    subtotals.Contains(XLSubtotalFunction.Variance),
                    false
                );
                xml.WriteAttributeDefault(
                    "varPSubtotal",
                    subtotals.Contains(XLSubtotalFunction.PopulationVariance),
                    false
                );

                foreach (uint fieldItem in reference.FieldItems)
                {
                    xml.WriteStartElement("x", Main2006SsNs);
    ...
    {
        string itemTypeAttr = itemType switch
        {
