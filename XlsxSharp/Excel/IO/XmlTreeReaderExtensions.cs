...
internal static class XmlTreeReaderExtensions
{
    ...
    public static bool TryReadColor(
        ...
        string ns,
        [NotNullWhen(true)] out XLColor? color
    )
    {
    ...
    public static XLColor ParseColor(this XmlTreeReader reader, string colorElementName, string ns)
    {
        ...
        uint? theme = reader.GetOptionalUInt("theme");
        if (theme is not null)
        {
            double tint = reader.GetOptionalDouble("tint") ?? 0;
            XLColor themeColor = XLColor.FromTheme((XLThemeColor)theme.Value, tint);
            reader.Close(colorElementName, ns);
            return themeColor;
        }

        string? rgb = reader.GetOptionalString("rgb");
        if (rgb is not null)
        {
            XLColor rgbColor = XLColor.FromColor(ColorStringParser.ParseFromArgb(rgb.AsSpan()));
            reader.Close(colorElementName, ns);
            return rgbColor;
        }

        int? indexed = reader.GetOptionalUintAsInt("indexed");
        if (indexed is not null)
        {
            XLColor indexedColor =
                indexed <= XLColor.IndexedColors.Count
                    ? XLColor.FromIndex(indexed.Value)
                    : XLColor.Automatic;
            reader.Close(colorElementName, ns);
            return indexedColor;
        }

        bool? auto = reader.GetOptionalBool("auto");
        if (auto is not null)
        {
            XLColor autoColor = XLColor.Automatic;
            reader.Close(colorElementName, ns);
            return autoColor;
        }
    ...
    /// </summary>
    public static bool TryReadBoolElement(
        this XmlTreeReader reader,
        string boolElementName,
        string ns,
        out bool value
    )
    {
    ...
    /// ]]>
    /// </summary>
    public static bool TryReadXStringValElement(
        this XmlTreeReader reader,
        string elementName,
        string ns,
    ...
    /// </summary>
    public static bool TryReadIntValElement(
        this XmlTreeReader reader,
        string elementName,
        string ns,
        out int value
    ...
    public static bool TryReadEnumValElement<TEnum>(
        this XmlTreeReader reader,
        string elementName,
        string ns,
