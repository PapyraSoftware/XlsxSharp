...
internal enum XLConnector
{
    And,
    Or,
}

internal enum XLFilterOperator
{
    Equal,
    NotEqual,
    GreaterThan,
...
internal class XLFilter
{
    ...
    internal static XLFilter CreateCustomPatternFilter(
        string filterValue,
        bool match,
        XLConnector connector
    )
    {
        ...
        XLCellValue testValue = XLCellValue.FromText(filterValue, CultureInfo.CurrentCulture);
        if (testValue.Type == XLDataType.Text)
        {
            ...
            return new XLFilter
            {
                ...
                Connector = connector,
                Condition = match
                    ? (c, _) => TextMatchesWildcard(wildcard, c)
                    : (c, _) => !TextMatchesWildcard(wildcard, c),
            };
        }
        ...
        return new XLFilter
        {
            ...
            Connector = connector,
            Condition = match
                ? (c, _) => ContentMatches(c, filterValue)
                : (c, _) =>
                    !CustomFilterSatisfied(
                        c.CachedValue,
        ...
        {
            XLCellValue cachedValue = cell.CachedValue;
            if (!cachedValue.IsText)
            {
                return false;
            }

            Wildcard wildcard = new(pattern);
            int position = wildcard.Search(cachedValue.GetText().AsSpan());
            return position >= 0;
        }
    }
    ...
    private static bool ContentMatches(IXLCell cell, string filterValue)
    {
        ...
        XLCellValue cachedValue = cell.CachedValue;
        string formattedString = ((XLCell)cell).GetFormattedString(cachedValue);
        return formattedString.Equals(filterValue, StringComparison.OrdinalIgnoreCase);
    }

    private static bool CustomFilterSatisfied(
        XLCellValue cellValue,
        XLFilterOperator op,
        XLCellValue filterValue,
        StringComparer textComparer
    )
    {
        ...
        int comparison = cellValue.Type switch
        {
    ...
    internal static XLFilter CreateTopBottom(bool takeTop, int percentsOrItemCount)
    {
        ...
        {
            XLCellValue cachedValue = cell.CachedValue;
            return cachedValue.IsUnifiedNumber
                && cachedValue.GetUnifiedNumber() >= filterColumn.TopBottomFilterValue;
        }
    ...
    internal static XLFilter CreateAverage(double initialAverage, bool aboveAverage)
    {
        ...
        {
            XLCellValue cachedValue = cell.CachedValue;
            double average = filterColumn.DynamicValue;
