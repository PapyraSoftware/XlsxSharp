...
internal readonly record struct Emu
{
    ...
    {
        int coef = GetUnitCoefficient(srcUnit);
        double emus = Math.Round(value * coef, MidpointRounding.AwayFromZero);
    ...
    {
        int coef = GetUnitCoefficient(unit);
    ...
    {
        double lengthInUnit = this.To(unit);
