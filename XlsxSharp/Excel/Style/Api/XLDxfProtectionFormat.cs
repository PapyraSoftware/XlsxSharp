...
internal class XLDxfProtectionFormat : IXLProtection
{
    ...
    public XLDxfProtectionFormat(XLDxFormat parent) => this._parent = parent;
    ...
    bool IEquatable<IXLProtection>.Equals(IXLProtection other) =>
        this.Locked == other.Locked && this.Hidden == other.Hidden;
    ...
    private T Resolve<T>(Func<XLDifferentialProtectionValue, T?> getProperty, T defaultValue)
        where T : struct =>
        this._parent.Resolve(static format => format.Protection, getProperty) ?? defaultValue;
