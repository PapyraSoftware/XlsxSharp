...
internal class FormatResolver
{
    ...
    {
        this._defaultFormat = worksheet.Workbook.Styles.DefaultFormat;
        this._worksheet = worksheet;
        this._columns = worksheet.Internals.ColumnsCollection;
        this._rows = worksheet.Internals.RowsCollection;
    }
    ...
    {
        if (this._rows.TryGetValue(point.Row, out XLRow? row) && row.FormatValue is { } rowFormat)
        {
            return rowFormat;
        }

        if (
            this._columns.TryGetValue(point.Column, out XLColumn? column)
        ...
        if (this._worksheet.FormatValue is { } worksheetFormat)
        {
