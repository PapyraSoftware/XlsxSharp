...
public partial class XLColor
{
    ...
    /// </summary>
    private static readonly Lazy<Dictionary<string, XLColor>> VmlNamedColors = new(() =>
        new Dictionary<string, XLColor>(StringComparer.OrdinalIgnoreCase)
        {
            { "Black", Black },
            { "Silver", Silver },
    ...
    {
        XLColorKey key = new() { ColorType = XLColorType.Color, Color = color };
    ...
    {
        XLColorKey key = new() { ColorType = XLColorType.Theme, ThemeColor = themeColor };
        return FromKey(ref key);
    }
    ...
    internal static XLColor FromVmlColor(String vmlColor)
    {
        ...
        if (
            rgbColorText.StartsWith("#".AsSpan())
            && ColorStringParser.TryParseRgb6(rgbColorText[1..], out Color rgbColor)
        )
        {
        ...
        string namedColorText = vmlColor.Trim();
        if (VmlNamedColors.Value.TryGetValue(namedColorText, out XLColor? namedColor))
        {
            return namedColor;
        }
        ...
        string paletteColorName = vmlColor.Split('[')[0].Trim();
        if (VmlPaletteColors.Value.TryGetValue(paletteColorName, out XLColor? paletteColor))
        {
            return paletteColor;
        }
