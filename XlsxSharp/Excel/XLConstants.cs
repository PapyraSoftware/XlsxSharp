...
public static class XLConstants
{
    ...
    {
        Dictionary<string, string> functionsMap = new(XlsxSharp.XLHelper.FunctionComparer);
        foreach (string futureFunction in XLConstants.FutureFunctions.Value)
        {
            functionsMap.Add(futureFunction, "_xlfn." + futureFunction);
        }

        foreach (string futureFunction in XLConstants.WorksheetOnlyFunctions.Value)
        {
