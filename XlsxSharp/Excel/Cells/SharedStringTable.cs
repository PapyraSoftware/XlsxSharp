...
internal class SharedStringTable
{
    ...
    /// </summary>
    internal int Count => this._table.Count - this._freeIds.Count;
    ...
    internal string this[int id]
    {
        get
        {
            object? potentialText = this._table[id].Text.Value;
    ...
    {
        object? text = this._table[id].Text.Value;
        if (text is null)
        {
            throw new ArgumentException($"Id {id} has no text.");
        }
    ...
    internal int IncreaseRef(string text, bool inline) =>
        this.IncreaseTextRef(new Text(text, inline));

    /// <inheritdoc cref="IncreaseRef(string, bool)"/>
    internal int IncreaseRef(XLImmutableRichText text, bool inline) =>
        this.IncreaseTextRef(new Text(text, inline));
    ...
    {
        Entry entry = this._table[id];
        if (entry.Text.Value is null)
        {
            throw new InvalidOperationException(
                "Trying to release a text that doesn't have a reference."
            );
        }

        if (entry.RefCount > 1)
        {
            this._table[id] = new Entry(entry.Text, entry.RefCount - 1);
            return;
        }

        this._table[id] = new Entry(Text.Empty, 0);
        this._freeIds.Add(id);
        this._reverseDict.Remove(entry.Text);
    }
    ...
    {
        List<int> map = new(this._table.Count);
        int mappedStringId = 0;
        for (int i = 0; i < this._table.Count; ++i)
        {
            Entry entry = this._table[i];
            bool isShared =
                entry.RefCount > 0
                && // Only used entry can be written to sst
    ...
    {
        foreach (Entry entry in this._table)
        {
            if (
                entry.RefCount > 0
    ...
    {
        if (!this._reverseDict.TryGetValue(text, out int id))
        {
            id = this.AddText(text);
            this._reverseDict.Add(text, id);
            return id;
        }
    ...
    {
        if (this._freeIds.Count > 0)
        {
            // List only changes size, not underlaying array, if last element is removed.
            int lastIndex = this._freeIds.Count - 1;
            int id = this._freeIds[lastIndex];
            this._freeIds.RemoveAt(lastIndex);
            this._table[id] = new Entry(text, 1);
            return id;
        }

        int lastTableIndex = this._table.Count;
        this._table.Add(new Entry(text, 1));
    ...
    private readonly struct Text : IEquatable<Text>
    {
        ...
        public override bool Equals(object? obj) => obj is Text other && this.Equals(other);
