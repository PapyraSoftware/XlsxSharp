...
internal sealed class XLCellFormula
{
    ...
    /// This property is meaningless, if called for non-data-table formula.
    /// </summary>
    internal Point Input1 => this._input1;
    ...
    /// This property is meaningless, if called for non-data-table formula.
    /// </summary>
    internal Point Input2 => this._input2;
    ...
    )
    {
        ...
        string formula = strValue.Trim();
        if (formula.StartsWith('='))
        {
            formula = formula[1..];
        }

        string converted = conversionType switch
        {
            FormulaConversionType.A1ToR1C1 => FormulaConverter.ToR1C1(
                formula,
                cellAddress.Row,
                cellAddress.Column
            ),
            FormulaConversionType.R1C1ToA1 => FormulaConverter.ToA1(
                formula,
                cellAddress.Row,
                cellAddress.Column
            ),
    ...
    )
    {
        ...
        string formula = string.Format(DataTableFormulaFormat, rowInput, colInput);
    ...
    {
        string colInput = input1Deleted ? "#REF!" : input1Address.ToString();
        string rowInput = input2Deleted ? "#REF!" : input2Address.ToString();
        string formula = string.Format(DataTableFormulaFormat, rowInput, colInput);
        return new XLCellFormula(formula)
        {
            Range = range,
    ...
    {
        Formula ast = engine.Parse(this.A1);
        return ast;
    }
    ...
    {
        string a1 = this.A1;
        string res = FormulaConverter.ModifyA1(
            a1,
            newSheetName,
            origin.Row,
            origin.Column,
    ...
    internal XLCellFormula GetMovedTo(Point origin, Point destination)
    {
        ...
        if (this.Type != FormulaType.Normal)
        {
            throw new InvalidOperationException("Can only swap normal formulas.");
        }

        string originR1C1 = FormulaConverter.ToR1C1(this.A1, origin.Row, origin.Column);
        string targetA1 = FormulaConverter.ToA1(originR1C1, destination.Row, destination.Column);
        XLCellFormula targetFormula = NormalA1(targetA1);
