...
internal class XLPivotTableField
{
    ...
    public XLPivotTableField(XLPivotTable pivotTable)
    {
        ...
        this.ShowAll = false; // The XML default value is true, but Excel always has false, so let's follow Excel.
        this.Subtotals = [XLSubtotalFunction.Automatic];
    }

    internal XLPivotTable PivotTable => this._pivotTable;

    /// <summary>
    /// Pivot field item, doesn't contain value, only indexes to <see cref="XLPivotCache"/> shared items.
    /// </summary>
    internal IReadOnlyList<XLPivotFieldItem> Items => this._items;
    ...
    internal void AddSubtotal(XLSubtotalFunction value)
    {
        ...
        XLPivotItemType subtotalItemType = GetItemTypeForSubtotal(value);
        ;
        this._items.Add(new XLPivotFieldItem(this, null) { ItemType = subtotalItemType });
    }

    internal void RemoveSubtotal(XLSubtotalFunction value)
    {
        this.Subtotals.Remove(value);
        XLPivotItemType subtotalItemType = GetItemTypeForSubtotal(value);
        this._items.RemoveAll(item => item.ItemType == subtotalItemType);
    }
    ...
    {
        int index = this._pivotTable.GetFieldIndex(this);
        XLPivotCache cache = this._pivotTable.PivotCache;
        XLPivotCacheValues cacheValues = cache.GetFieldValues(index);
        int sharedItemIndex = cacheValues.GetOrAddSharedItem(value);

        // Excel tries to repair workbook, when there are duplicates in pivotFields.Items
        // therefore add only if necessary
        XLPivotFieldItem? existingItem = this._items.FirstOrDefault(x =>
            x.ItemIndex == sharedItemIndex
        );
        ...
        XLPivotFieldItem newItem = new(this, sharedItemIndex);
    ...
    {
        int fieldIndex = this._pivotTable.GetFieldIndex(this);
        XLPivotCacheSharedItems sharedItems = this._pivotTable.PivotCache.GetFieldSharedItems(
            fieldIndex
        );

        Dictionary<uint, int> pivotFieldItems = this
            .Items.WhereNotNull(fieldItem => fieldItem.ItemIndex)
            .Select((itemItem, index) => (Index: index, ItemIndex: (uint)itemItem))
            .ToDictionary(x => x.ItemIndex, x => x.Index);
        List<XLPivotFieldItem> filteredItems = [];
        for (uint sharedItemIndex = 0; sharedItemIndex < sharedItems.Count; ++sharedItemIndex)
        {
            XLCellValue sharedItemValue = sharedItems[sharedItemIndex];
            ...
            if (pivotFieldItems.TryGetValue(sharedItemIndex, out int index))
            {
                filteredItems.Add(this.Items[index]);
            }
            else
            {
                XLPivotFieldItem addedItem = new(this, (int)sharedItemIndex);
    ...
    {
        XLPivotItemType subtotalItemType = value switch
        {
