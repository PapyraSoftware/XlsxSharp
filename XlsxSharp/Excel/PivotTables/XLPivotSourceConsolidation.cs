...
internal sealed class XLPivotSourceConsolidation : IXLPivotSource
{
    ...
    /// </para>
    /// </summary>
    public IReadOnlyList<XLPivotCacheSourceConsolidationPage> Pages { get; init; } =
        Array.Empty<XLPivotCacheSourceConsolidationPage>();

    /// <summary>
    /// Range sets that consists the cache source.
    /// </summary>
    public IReadOnlyList<XLPivotCacheSourceConsolidationRangeSet> RangeSets { get; init; } =
        Array.Empty<XLPivotCacheSourceConsolidationRangeSet>();

    public bool Equals(IXLPivotSource otherSource)
    {
        XLPivotSourceConsolidation? other = otherSource as XLPivotSourceConsolidation;
        if (other is null)
        {
            return false;
        }
