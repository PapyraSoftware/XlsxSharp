...
internal static class TimeSpanParser
{
    public static bool TryParseTime(string s, CultureInfo ci, out TimeSpan result)
    {
        ...
        if (!TryReadNumber(ref i, s, out int hoursOrMinutes))
        {
            return false;
        }
        ...
        if (!TryReadNumber(ref i, s, out int minutesOrSeconds))
        {
            return false;
        }
        ...
        {
            SkipWhitespace(ref i, s);
            int ms = ReadFractionInMs(ref i, s); // '10:20.' is allowed without digits
            SkipWhitespace(ref i, s);
            result = new TimeSpan(0, 0, hoursOrMinutes, minutesOrSeconds, ms);
            return i == s.Length && (hoursOrMinutes < 60 || minutesOrSeconds < 60); // No check for min/sec over limit
        }

        // Longer path for h:m:s[.f]
        if (!InputMatches(ref i, s, timeSeparator)) // There is some other character after '10:00', but only ':' ('10:20:0')
        {
            return false;
        }
        ...
        if (!TryReadNumber(ref i, s, out int seconds)) // Seconds
        {
            return false;
        }
        ...
        if (
            (hoursOrMinutes >= 24 && minutesOrSeconds >= 60)
        ...
        if (!InputMatches(ref i, s, decimalSeparator)) // The only allowed character is a decimal separator for '1:0:0.'
        {
            return false;
        }

        SkipWhitespace(ref i, s);
        int milliseconds = ReadFractionInMs(ref i, s);
        SkipWhitespace(ref i, s);
        ...
        static int ReadFractionInMs(ref int i, string t)
        {
            ...
            return (int)
                Math.Round(num / Math.Pow(10, digitCount - 3), MidpointRounding.AwayFromZero);
        }
        ...
        {
            for (int expectedIdx = 0; expectedIdx < expected.Length; ++expectedIdx)
            {
                int inputIdx = i + expectedIdx;
