...
namespace XlsxSharp.Excel.CalcEngine
{
    ...
    internal class Reference
    {
        ...
        public IEnumerable<ScalarValue> GetCellsValues(CalcContext ctx)
        {
            ...
            {
                for (
                    var row = area.FirstAddress.RowNumber;
                    row <= area.LastAddress.RowNumber;
                    ++row
                )
                {
                    for (
                        var column = area.FirstAddress.ColumnNumber;
                        column <= area.LastAddress.ColumnNumber;
                        ++column
                    )
                    {
        ...
        public static OneOf<Reference, XLError> RangeOp(
            Reference lhs,
            Reference rhs,
            XLWorksheet contextWorksheet
        )
        {
            var lhsWorksheets =
                lhs.Areas.Count == 1
                    ...
                    : lhs
                        .Areas.Select(a => a.Worksheet ?? contextWorksheet)
            ...
            var minCol = XlsxSharp.XLHelper.MaxColumnNumber;
            var maxCol = 1;
            var minRow = XlsxSharp.XLHelper.MaxRowNumber;
            var maxRow = 1;
        ...
        public static OneOf<Reference, XLError> Intersect(
            Reference lhs,
            Reference rhs,
            CalcContext ctx
        )
        {
            var sheets = lhs
                .Areas.Select(a => a.Worksheet ?? ctx.Worksheet)
        ...
        public OneOf<Reference, XLError> ImplicitIntersection(IXLAddress formulaAddress)
        {
            ...
            if (
                area.ColumnSpan == 1
                && area.FirstAddress.RowNumber <= row
                && row <= area.LastAddress.RowNumber
            )
            {
                var intersection = new XLAddress(
                    area.Worksheet,
                    row,
            ...
            if (
                area.RowSpan == 1
                && area.FirstAddress.ColumnNumber <= column
                && column <= area.LastAddress.ColumnNumber
            )
            {
                var intersection = new XLAddress(
