...
internal abstract class AstNode
{
    ...
    public abstract TResult Accept<TContext, TResult>(
        TContext context,
        IFormulaVisitor<TContext, TResult> visitor
    );
}
...
{
    public FunctionNode(string name, IReadOnlyList<ValueNode> parms)
        : this(null, name, parms) { }
...
/// <summary>
/// An placeholder node for AST nodes that are not yet supported in XlsxSharp.
/// </summary>
...
internal class PrefixNode : AstNode
{
    ...
    internal OneOf<IXLWorksheet, XLError> GetWorksheet(XLWorkbook wb)
    {
        ...
        if (this.FirstSheet is not null || this.LastSheet is not null)
        {
            throw new NotImplementedException("3D references are not yet implemented.");
        }

        if (!wb.TryGetWorksheet(this.Sheet, out XLWorksheet worksheet))
        {
...
internal class ReferenceNode : ValueNode
{
    public ReferenceNode(PrefixNode? prefix, ReferenceArea referenceArea, bool isA1)
    {
        this.Prefix = prefix;
        this.Address = isA1
            ? referenceArea.GetDisplayStringA1()
            : referenceArea.GetDisplayStringR1C1();
    ...
    public AnyValue GetReference(CalcContext ctx)
    {
        ...
        {
            return new Reference(new XLRangeAddress(null, this.Address));
        }

        if (
            !this.Prefix.GetWorksheet(ctx.Workbook).TryPickT0(out IXLWorksheet? ws, out XLError err)
        )
        {
            return err;
        }
        ...
        return new Reference(new XLRangeAddress((XLWorksheet)ws!, this.Address));
    }
}
...
internal class NameNode : ValueNode
{
    ...
    {
        XLWorksheet worksheet = ctxWs;
        if (this.Prefix is not null)
        {
            if (
                !this
                    .Prefix.GetWorksheet(ctxWs.Workbook)
                    .TryPickT0(out IXLWorksheet? ws, out XLError err)
            )
            {
                return err;
            }
        ...
        if (!this.TryGetNameRange(worksheet, out IXLDefinedName? definedName))
        {
    ...
    internal bool TryGetNameRange(
        IXLWorksheet ws,
        [NotNullWhen(true)] out IXLDefinedName? definedName
    )
    {
        if (ws.DefinedNames.TryGetValue(this.Name, out IXLDefinedName? sheetDefinedName))
        {
        ...
        if (
            ws.Workbook.DefinedNamesInternal.TryGetValue(
                this.Name,
                out IXLDefinedName? bookDefinedName
            )
        )
        {
