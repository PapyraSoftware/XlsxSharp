...
internal readonly struct ScalarValue
{
    ...
    public static ScalarValue From(bool logical) =>
        new(LogicalValue, logical, default, default, default);

    public static ScalarValue From(double number) =>
        new(NumberValue, default, number, default, default);
    ...
    public static ScalarValue From(XLError error) =>
        new(ErrorValue, default, default, default, error);
    ...
    public TResult Match<TResult, TParam1>(
        TParam1 param,
        Func<TParam1, TResult> transformBlank,
        Func<bool, TParam1, TResult> transformLogical,
        ...
        this._index switch
        {
            ...
            NumberValue => transformNumber(this._number, param),
            TextValue => transformText(this._text, param),
    ...
    public TResult Match<TResult, TParam1, TParam2>(
        TParam1 param1,
        TParam2 param2,
        Func<TParam1, TParam2, TResult> transformBlank,
        Func<bool, TParam1, TParam2, TResult> transformLogical,
        ...
        this._index switch
        {
            ...
            NumberValue => transformNumber(this._number, param1, param2),
            TextValue => transformText(this._text, param1, param2),
    ...
    public OneOf<string, XLError> ToText(CultureInfo culture) =>
        this._index switch
        {
            ...
            NumberValue => this._number.ToString(culture),
            TextValue => this._text,
            ErrorValue => this._error,
            _ => throw new InvalidOperationException(),
        };
    ...
    public OneOf<double, XLError> ToNumber(CultureInfo culture) =>
        this._index switch
        {
            ...
            NumberValue => this._number,
            TextValue => TextToNumber(this._text, culture),
            ErrorValue => this._error,
            _ => throw new InvalidOperationException(),
        };
    ...
    public static ScalarValue Parse(string text, CultureInfo culture)
    {
        ...
        if (TextToNumber(text, culture).TryPickT0(out double number, out _))
        {
            return number;
        }
    ...
    public static OneOf<double, XLError> TextToNumber(string text, CultureInfo culture)
    {
        ...
        //       11 '0.00E+00'
        //       48 '##0.0E+0'
        if (double.TryParse(text, NumberStyles.Any, culture, out double number))
        {
            return number;
        }
        ...
        // Format 9 '0%'
        //       10 '0.00%'
        ReadOnlySpan<char> textSpan = text.AsSpan(); // Avoid extra allocations for trimming/substrings if not match
        ReadOnlySpan<char> textSpanTrimmedEnd = textSpan.TrimEnd();
        ReadOnlySpan<char> percentSymbol = culture.NumberFormat.PercentSymbol.AsSpan();
        ...
        ReadOnlySpan<char> textSpanTrimmedStart = textSpan.TrimStart();
        if (textSpanTrimmedStart.StartsWith(percentSymbol))
        {
            int newStart = text.Length - textSpanTrimmedStart.Length + percentSymbol.Length;
        ...
        // Format 12 '# ?/?'
        //        13 '# ??/??'
        if (FractionParser.TryParse(text, out double fraction))
        {
            return fraction;
        }

        if (ToSerialDateTime(text, culture, out double serialDateTime))
        {
        ...
        static OneOf<double, XLError> ParsePercent(
            string text,
            int start,
            int length,
            CultureInfo c
    ...
    {
        const DateTimeStyles dateStyle =
        ...
        if (DateTimeParser.TryParseCultureDate(text, culture, out DateTime dateFormat14Or22))
        {
        ...
        if (
            DateTime.TryParseExact(
                text,
                ["d-MMM-yyyy", "d-MMMM-yyyy", "d-MMM-yy", "d-MMMM-yy"],
                culture,
                dateStyle,
                out DateTime dateFormat15
            )
        )
        {
        ...
        if (DateTimeParser.TryParseTimeOfDay(text, culture, out DateTime dateFormat18Or19))
        {
        ...
        //        21 'h:mm:ss'
        //        47 'mm:ss.0'
        if (TimeSpanParser.TryParseTime(text, culture, out TimeSpan timeSpan))
        {
        ...
        static bool ToSerialDate(DateTime dateTime, out double serialDate)
        {
            ...
            double oDate = dateTime.ToOADate();
    ...
    public bool TryPickNumber(out double number, out XLError error)
    {
        ...
        number = default;
        error = this.IsError ? this._error : XLError.IncompatibleValue;
        return false;
    }
    ...
    /// </summary>
    public bool HaveSameType(ScalarValue other) => this._index == other._index;
    ...
    public bool TryCoerceLogicalOrBlankOrNumberOrText(out bool value, out XLError error)
    {
        switch (this._index)
        {
            ...
            case LogicalValue:
                ...
                return true;
            case NumberValue:
                value = this._number != 0;
                error = default;
                return true;
            case TextValue when (StringComparer.OrdinalIgnoreCase.Equals(this._text, "TRUE")):
                value = true;
                error = default;
                return true;
            case TextValue when (StringComparer.OrdinalIgnoreCase.Equals(this._text, "FALSE")):
                value = false;
                error = default;
                return true;
            case ErrorValue:
    ...
    public override string ToString() =>
        this._index switch
        {
            ...
            NumberValue => this._number.ToString(CultureInfo.InvariantCulture),
            TextValue => this._text,
