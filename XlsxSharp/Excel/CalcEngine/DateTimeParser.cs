...
internal static class DateTimeParser
{
    ...
    private static readonly ConcurrentDictionary<CultureInfo, string[]> CultureSpecificPatterns =
        new();
    ...
    public static bool TryParseCultureDate(string s, CultureInfo culture, out DateTime date)
    {
        string[] datePatterns = CultureSpecificPatterns.GetOrAdd(
            ...
            static ci =>
            {
                ...
                string[] timePatterns = ["h:m tt", "H:m", "h:m"];
                IEnumerable<string> longDatePatterns = shortDatePatterns.SelectMany(datePattern =>
                    timePatterns.Select(timePattern =>
                        FormattableString.Invariant($"{datePattern} {timePattern}")
                    )
                );
