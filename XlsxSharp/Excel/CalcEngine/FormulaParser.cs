...
internal class FormulaParser
{
    ...
    {
        this._nodeFactoryA1 = new AstFactory(functionRegistry, true);
        this._nodeFactoryR1C1 = new AstFactory(functionRegistry, false);
    }
    ...
    private sealed class AstFactory : IAstFactory<ScalarValue, ValueNode, string>
    {
        ...
        {
            LiteralArray array = new(rows, columns, elements);
        ...
        {
            XLError error = GetErrorValue(errorText);
        ...
        {
            PrefixNode prefixNode = new(null, sheet, null, null);
        ...
        {
            PrefixNode prefixNode = new(null, null, firstSheet, lastSheet);
            return new ReferenceNode(prefixNode, area, this._isA1);
        }

        public ValueNode ExternalSheetReference(
            string context,
            SymbolRange range,
            int workbookIndex,
            string sheet,
        ...
        {
            FileNode fileNode = new(workbookIndex);
            PrefixNode prefixNode = new(fileNode, sheet, null, null);
            return new ReferenceNode(prefixNode, area, this._isA1);
        }

        public ValueNode ExternalReference3D(
            string context,
            SymbolRange range,
            int workbookIndex,
            string firstSheet,
        ...
        {
            FileNode fileNode = new(workbookIndex);
            PrefixNode prefixNode = new(fileNode, null, firstSheet, lastSheet);
            return new ReferenceNode(prefixNode, area, this._isA1);
        }
        ...
        )
        {
            ...
            return this.GetFunctionNode(null, functionName, args);
        }
        ...
        {
            PrefixNode prefixNode = new(null, sheetName, null, null);
            return this.GetFunctionNode(prefixNode, name.ToString(), args);
        }

        public ValueNode ExternalFunction(
            string context,
            SymbolRange range,
            int workbookIndex,
            string sheet,
        ...
        {
            PrefixNode prefixNode = new(new FileNode(workbookIndex), sheet, null, null);
            return this.GetFunctionNode(prefixNode, name.ToString(), args);
        }
        ...
        {
            PrefixNode prefixNode = new(new FileNode(workbookIndex), null, null, null);
            return this.GetFunctionNode(prefixNode, name.ToString(), args);
        }

        public ValueNode CellFunction(
            string context,
            SymbolRange range,
            RowCol cell,
            IReadOnlyList<ValueNode> args
        )
        {
            ...
            // here.
            string functionName = context.Substring(
                range.Start,
                context.IndexOf('(', range.Start) - range.Start
            );
            if (this._functionRegistry.TryGetFunc(functionName, out _, out _))
            {
        ...
        public ValueNode ExternalSheetName(
            string context,
            SymbolRange range,
            int workbookIndex,
            string sheet,
        ...
        {
            bool foundFunction = this._functionRegistry.TryGetFunc(
                functionName,
                out int minParams,
                out int maxParams
            );
            ...
            if (!foundFunction && functionName.StartsWith($"{DefaultFunctionNameSpace}."))
            {
                ...
                foundFunction = this._functionRegistry.TryGetFunc(
                    functionName,
                    out minParams,
                    out maxParams
                );
        ...
        {
            if (!XLErrorParser.TryParseError(error.ToString(), out XLError errorEnum))
            {
