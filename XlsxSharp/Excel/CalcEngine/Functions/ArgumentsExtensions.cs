    ...
    )
    {
        ...
        foreach (AnyValue arg in args)
        {
            if (arg.TryPickScalar(out ScalarValue scalar, out OneOf<Array, Reference> collection))
            {
                ...
                OneOf<TValue, XLError> conversionResult = convert(scalar, ctx);
                if (!conversionResult.TryPickT0(out TValue? elementValue, out XLError elementError))
                {
                    return elementError;
                }
            ...
            else
            {
                ...
                foreach (ScalarValue value in valuesIterator)
                {
                    ...
                    OneOf<TValue, XLError> conversionResult = convert(value, ctx);
                    if (
                        !conversionResult.TryPickT0(
                            out TValue? elementValue,
                            out XLError elementError
                        )
                    )
                    {
                        return elementError;
