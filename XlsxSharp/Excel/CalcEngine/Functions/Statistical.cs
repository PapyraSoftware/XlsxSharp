...
internal static class Statistical
{
    public static void Register(FunctionRegistry ce)
    {
        ...
        ce.RegisterFunction(
            ...
            2,
            2,
            Adapt((Func<CalcContext, AnyValue, ScalarValue, AnyValue>)CountIf),
            FunctionFlags.Range,
            AllowRange.Only,
            0
        );
        ce.RegisterFunction(
            ...
            FunctionFlags.Range,
            AllowRange.Only,
            [.. Enumerable.Range(0, 128).Select(x => x * 2)]
        );
    ...
    internal static AnyValue Average(CalcContext ctx, Span<AnyValue> args, ITally tally)
    {
        ...
        if (
            !tally.Tally(ctx, args, new SumState()).TryPickT0(out SumState state, out XLError error)
        )
        {
            return error;
        }
    ...
    )
    {
        ...
        {
            double cdf = 0d;
            for (int y = 0; y <= numberSuccesses; ++y)
            {
                OneOf<double, XLError> result = BinomDist(ctx, y, numberTrials, successProbability);
                if (!result.TryPickT0(out double pf, out XLError error))
                {
                    return error;
                }
        ...
        {
            OneOf<double, XLError> result = BinomDist(
                ctx,
                numberSuccesses,
                numberTrials,
                successProbability
            );
            if (!result.TryPickT0(out double binomDist, out XLError error))
            {
                return error;
            }
    ...
    {
        if (!XLMath.CombinChecked(ctx, n, x).TryPickT0(out double combinations, out XLError error))
        {
            return error;
        }
        ...
        x = Math.Floor(x);
        n = Math.Floor(n);
        double binomDist = combinations * Math.Pow(p, x) * Math.Pow(1 - p, n - x);
    ...
    internal static AnyValue Count(CalcContext ctx, Span<AnyValue> args, ITally tally)
    {
        ...
        OneOf<CountState, XLError> result = tally.Tally(ctx, args, new CountState(0));
        if (!result.TryPickT0(out CountState state, out XLError error))
        {
            return error;
        }
    ...
    {
        if (!arg.TryPickArea(out XLRangeAddress area, out XLError error))
        {
            return error;
        }
        ...
        long nonBlankCount = 0L;
        foreach (ScalarValue value in ctx.GetNonBlankValues(new Reference(area)))
        {
    ...
    private static AnyValue CountIf(
        CalcContext ctx,
        AnyValue countRange,
    ...
    )
    {
        ...
        if (!countRange.TryPickArea(out XLRangeAddress countArea, out XLError areaError))
        {
            return areaError;
        }

        TallyCriteria tally = new(static _ => 1);
        ...
        OneOf<CountState, XLError> result = tally.Tally(
            ctx,
            new[] { countRange },
            new CountState(0)
        );
    ...
    private static AnyValue CountIfs(
        CalcContext ctx,
    ...
    )
    {
        if (
            !criteriaRanges[0]
                .Range.TryPickArea(out XLRangeAddress countArea, out XLError areaError)
        )
        {
            return areaError;
        }
        ...
        foreach ((AnyValue selectionRange, ScalarValue selectionCriteria) in criteriaRanges)
        {
            ...
            if (
                countArea.RowSpan != selectionArea.RowSpan
    ...
    {
        OneOf<SquareDiff, XLError> result = GetSquareDiffSum(ctx, args, TallyNumbers.Default);
        if (!result.TryPickT0(out SquareDiff squareDiff, out XLError error))
        {
            return error;
        }
    ...
    private static AnyValue GeoMean(CalcContext ctx, Span<AnyValue> args)
    {
        ...
        OneOf<LogSumState, XLError> tally = TallyNumbers.Default.Tally(
            ctx,
            args,
            new LogSumState(0.0, 0)
        );
        if (!tally.TryPickT0(out LogSumState geoMean, out XLError error))
        {
            return error;
        }
    ...
    {
        OneOf<MaxState, XLError> result = tally.Tally(ctx, args, new MaxState());
        if (!result.TryPickT0(out MaxState state, out XLError error))
        {
            return error;
        }
    ...
    private static AnyValue Median(CalcContext ctx, Span<AnyValue> args)
    {
        ...
        OneOf<ValuesState, XLError> result = TallyNumbers.Default.Tally(
            ctx,
            args,
            new ValuesState([])
        );
        if (!result.TryPickT0(out ValuesState state, out XLError error))
        {
            return error;
        }

        List<double> allNumbers = state.Values;
        ...
        int halfIndex = allNumbers.Count / 2;
        bool hasEvenCount = allNumbers.Count % 2 == 0;
        if (hasEvenCount)
        {
    ...
    {
        OneOf<MinState, XLError> result = tally.Tally(ctx, args, new MinState());

        if (!result.TryPickT0(out MinState state, out XLError error))
        {
            return error;
        }
    ...
    {
        if (
            !GetSquareDiffSum(ctx, args, tally)
                .TryPickT0(out SquareDiff squareDiff, out XLError error)
        )
        {
            return error;
        }
    ...
    private static AnyValue Large(CalcContext ctx, AnyValue arrayParam, double kParam)
    {
        ...
        int k = (int)Math.Ceiling(kParam);

        IEnumerable<ScalarValue> values;
        int size;
        if (
            arrayParam.TryPickScalar(out ScalarValue scalar, out OneOf<Array, Reference> collection)
        )
        {
        ...
        List<double> total = new(size);
        foreach (ScalarValue value in values)
        {
            ctx.ThrowIfCancelled();
            if (value.IsError)
            {
    ...
    /// </summary>
    private static OneOf<SquareDiff, XLError> GetSquareDiffSum(
        CalcContext ctx,
        Span<AnyValue> args,
        ITally tally
    )
    {
        if (
            !tally
                .Tally(ctx, args, new SumState(0.0, 0))
                .TryPickT0(out SumState sumState, out XLError sumError)
        )
        {
            return sumError;
        }
        ...
        double sampleMean = sumState.Sum / sumState.Count;

        // Calculate sum of squares of deviations from sample mean
        SquareDiff initialSquareDiffState = new(Sum: 0.0, Count: 0, SampleMean: sampleMean);
        OneOf<SquareDiff, XLError> result = tally.Tally(ctx, args, initialSquareDiffState);
    ...
    {
        public SumState Tally(double number) => new(this.Sum + number, this.Count + 1);
    }

    private readonly record struct SquareDiff(double Sum, int Count, double SampleMean)
        : ITallyState<SquareDiff>
    {
        public SquareDiff Tally(double sampleValue)
        {
            double diff = sampleValue - this.SampleMean;
            double sum = this.Sum + diff * diff;
            return new SquareDiff(sum, this.Count + 1, this.SampleMean);
        }
    }
    ...
    {
        public MinState()
            : this(double.MaxValue, false) { }

        public MinState Tally(double number) => new(Math.Min(this.Min, number), true);
    }

    private readonly record struct MaxState(double Max, bool HasValues) : ITallyState<MaxState>
    {
        public MaxState()
            : this(double.MinValue, false) { }

        public MaxState Tally(double number) => new(Math.Max(this.Max, number), true);
    }
    ...
    private readonly record struct LogSumState(double LogSum, int Count) : ITallyState<LogSumState>
    {
        ...
        {
            double logSum = this.LogSum + Math.Log(number);
            return new LogSumState(logSum, this.Count + 1);
        }
    }
    ...
    private readonly record struct ValuesState(List<double> Values) : ITallyState<ValuesState>
    {
        public ValuesState Tally(double number)
        {
            ...
            return new ValuesState(this.Values);
        }
    }

    private readonly record struct CountState(int Count) : ITallyState<CountState>
    {
        public CountState Tally(double number) => new(this.Count + 1);
    }
}
