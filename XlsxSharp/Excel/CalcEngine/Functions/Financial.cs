...
internal static class Financial
{
    ...
    private static double FvInternal(
        double rate,
        double numberOfPayments,
        double pmt,
        double presentValue,
    ...
    )
    {
        ...
        return -(
            pmt * (Math.Pow(1 + rate, numberOfPayments) - 1) / rate
            + presentValue * Math.Pow(1 + rate, numberOfPayments)
        );
    }

    private static AnyValue Ipmt(
        double rate,
        double period,
        double numberOfPayments,
        double presentValue,
    ...
    )
    {
        ...
        double ipmt =
            FvInternal(
                rate,
                period - 1,
                PmtInternal(rate, numberOfPayments, presentValue, futureValue, type),
                presentValue,
                type
            ) * rate;
    ...
    private static double PmtInternal(
        double rate,
        double numberOfPayments,
        double presentValue,
        double futureValue,
    ...
    )
    {
        ...
        int timingOffset = type != 0.0 ? paymentAtTheBeginningOfPeriod : paymentAtTheEndOfPeriod;

        return (-futureValue - presentValue * Math.Pow(1.0 + rate, numberOfPayments))
            / (1 + rate * timingOffset)
            / ((Math.Pow(1.0 + rate, numberOfPayments) - 1) / rate);
    }
}
