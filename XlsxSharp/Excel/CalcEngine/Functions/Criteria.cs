...
internal class Criteria
{
    ...
    internal bool CanBlankValueMatch
    {
        get
        {
            ...
            if (this._comparison is Comparison.Equal or Comparison.None && this._value.IsBlank)
            {
                return true;
            }
            ...
            if (this._comparison == Comparison.NotEqual && !this._value.IsBlank)
            {
                return true;
            }
    ...
    internal static Criteria Create(ScalarValue criteria, CultureInfo culture)
    {
        if (criteria.IsText)
        {
            ...
            (string prefix, Comparison comparison) = GetComparison(criteriaText);
            string operandText = criteriaText[prefix.Length..];
            ScalarValue operand = ScalarValue.Parse(operandText, culture);
        ...
        {
            foreach ((string prefix, Comparison prefixComparison) in AllComparisons)
            {
    ...
    internal bool Match(ScalarValue value) =>
        this._value switch
        {
            ...
            { IsNumber: true } => this.CompareNumber(value, this._value.GetNumber()),
            { IsText: true } => this.CompareText(value, this._value.GetText()),
    ...
    private bool CompareLogical(ScalarValue value, bool actual)
    {
        if (!value.IsLogical)
        {
            return this._comparison == Comparison.NotEqual;
        }

        return this.Compare(value.GetLogical().CompareTo(actual));
    }
    ...
    private bool CompareNumber(ScalarValue value, double actual)
    {
        ...
        if (value.IsNumber)
        {
            ...
            value.IsText
            && ScalarValue
                .TextToNumber(value.GetText(), this._culture)
                .TryPickT0(out double parsedNumber, out _)
        )
        {
            number = parsedNumber;
        }
        ...
        return this.Compare(number.CompareTo(actual));
    }
    ...
    private bool CompareText(ScalarValue value, string actual)
    {
        ...
        {
            Comparison.Equal or Comparison.None => new Wildcard(actual).Matches(
                value.GetText().AsSpan()
            ),
            Comparison.NotEqual => !new Wildcard(actual).Matches(value.GetText().AsSpan()),
            Comparison.LessThan => this._culture.CompareInfo.Compare(value.GetText(), actual) < 0,
            Comparison.LessOrEqualTo => this._culture.CompareInfo.Compare(value.GetText(), actual)
                <= 0,
            Comparison.GreaterThan => this._culture.CompareInfo.Compare(value.GetText(), actual)
                > 0,
            Comparison.GreaterOrEqualTo => this._culture.CompareInfo.Compare(
                value.GetText(),
                actual
            ) >= 0,
            _ => throw new UnreachableException(),
