...
internal class TallyAll : ITally
{
    ...
    /// </summary>
    internal static readonly ITally Subtotal10 = new TallyAll(
        getNonBlankValues: static (ctx, reference) =>
            ctx.GetFilteredNonBlankValues(reference, "SUBTOTAL")
    );

    /// <summary>
    /// Tally algorithm for <c>SUBTOTAL</c> functions 101..111.
    /// </summary>
    internal static readonly ITally Subtotal100 = new TallyAll(
        getNonBlankValues: static (ctx, reference) =>
            ctx.GetFilteredNonBlankValues(reference, "SUBTOTAL", skipHiddenRows: true)
    );

    private TallyAll(
        bool ignoreArrayText = true,
        bool includeErrors = false,
        Func<CalcContext, Reference, IEnumerable<ScalarValue>>? getNonBlankValues = null
    )
    {
        this._ignoreArrayText = ignoreArrayText;
        this._includeErrors = includeErrors;
        ...
        where T : ITallyState<T>
    {
        ...
        foreach (AnyValue arg in args)
        {
            ...
            else
            {
                ...
                {
                    valuesIterator = this._getNonBlankValues(ctx, reference);
                    isArray = false;
                }
                foreach (ScalarValue value in valuesIterator)
                {
                    ...
                    if (!isArray && value.TryPickLogical(out bool logical))
                    {
                        state = state.Tally(logical ? 1 : 0);
                    }
                    else if (value.TryPickNumber(out double number))
                    {
                        state = state.Tally(number);
                    }
                    else if (value.IsText && (!isArray || !this._ignoreArrayText))
                    {
                    ...
                    else if (value.TryPickError(out XLError error))
                    {
                        if (!this._includeErrors)
                        {
                            return error;
