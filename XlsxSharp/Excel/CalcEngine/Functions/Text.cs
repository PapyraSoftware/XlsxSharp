...
internal static class Text
{
    ...
    private static readonly Lazy<Dictionary<int, string>> Windows1252Char = new(static () =>
        Enumerable
            .Range(0, 0x80)
            .Select(static i => (Char: (char)i, Code: i))
    ...
    public static void Register(FunctionRegistry ce)
    {
        ...
        ce.RegisterFunction(
            ...
            255,
            Adapt(Concat),
            FunctionFlags.Future | FunctionFlags.Range,
            AllowRange.All
        ...
        ce.RegisterFunction(
            ...
            1,
            3,
            AdaptLastTwoOptional(Fixed, 2, false),
        ...
        ce.RegisterFunction(
            ...
            255,
            Adapt(TextJoin),
            FunctionFlags.Range | FunctionFlags.Future,
            AllowRange.Except,
            0,
            1
        ); // Joins text via delimiter
    ...
    private static ScalarValue Code(CalcContext ctx, string text)
    {
        ...
        if (!Windows1252Code.Value.TryGetValue(text[0], out int code))
        {
    ...
    private static ScalarValue Concat(CalcContext ctx, List<Array> texts)
    {
        ...
        foreach (Array array in texts)
        {
            foreach (ScalarValue scalar in array)
            {
                ctx.ThrowIfCancelled();
                if (!scalar.ToText(ctx.Culture).TryPickT0(out string? text, out XLError error))
                {
                    return error;
                }
    ...
    {
        int totalLength = texts.Sum(static x => x.Length);
    ...
    {
        int startIndex = startNum.TryPickT0(out double startNumber, out _)
            ? (int)Math.Truncate(startNumber) - 1
            : 0;
        ...
        ReadOnlySpan<char> text = withinText.AsSpan(startIndex);
        int index = text.IndexOf(findText.AsSpan());
        return index == -1 ? XLError.IncompatibleValue : index + startIndex + 1;
    }

    private static ScalarValue Fixed(
        CalcContext ctx,
        double number,
        double numDecimals,
        bool suppressComma
    )
    {
        ...
        CultureInfo culture = ctx.Culture;
        if (suppressComma)
        {
        ...
        double rounded = XLMath.Round(number, numDecimals);

        // Number rounded to tens, hundreds... should be displayed without any decimal places
        double digits = Math.Max(numDecimals, 0);
    ...
    private static ScalarValue Mid(CalcContext ctx, string text, double startPos, double numChars)
    {
        ...
        int start = checked((int)Math.Truncate(startPos)) - 1;
        int length = checked((int)Math.Truncate(numChars));
    ...
    private static ScalarValue Proper(CalcContext ctx, string text)
    {
        ...
        {
            ctx.ThrowIfCancelled();
            char casedChar = prevWasLetter ? char.ToLower(c, culture) : char.ToUpper(c, culture);
            sb.Append(casedChar);
    ...
    private static ScalarValue Replace(
        CalcContext ctx,
        string oldText,
        double startPos,
        double numChars,
    ...
    {
        if (startPos is < 1 or >= XlsxSharp.XLHelper.CellTextLimit + 1)
        {
            return XLError.IncompatibleValue;
        }

        if (numChars is < 0 or >= XlsxSharp.XLHelper.CellTextLimit + 1)
        {
            return XLError.IncompatibleValue;
        }

        int prefixLength = checked((int)startPos) - 1;
        if (prefixLength > oldText.Length)
        {
            prefixLength = oldText.Length;
        }

        int deletedLength = checked((int)numChars);
        ...
        StringBuilder sb = new(oldText.Length - deletedLength + replacement.Length);
        ReadOnlySpan<char> text = oldText.AsSpan();
    ...
    private static ScalarValue Rept(CalcContext ctx, string text, double replicationCount)
    {
        ...
        int count = checked((int)replicationCount);
        int resultLength = text.Length * count;
        if (resultLength > XlsxSharp.XLHelper.CellTextLimit)
        {
            return XLError.IncompatibleValue;
        }
    ...
    )
    {
        ...
        Wildcard wildcard = new(findText);
        ReadOnlySpan<char> text = withinText.AsSpan().Slice(startIndex);
        int firstIdx = wildcard.Search(text);
        if (firstIdx < 0)
        {
            return XLError.IncompatibleValue;
        }
    ...
    private static ScalarValue Substitute(
        CalcContext ctx,
        string text,
        string oldText,
        string newText,
    ...
    )
    {
        ...
        int pos = -1;
        int occurrence = (int)occurrenceOrMissing.Value;
        ...
        StringBuilder sb = new(text.Length - oldText.Length + newText.Length);
    ...
    private static AnyValue T(CalcContext ctx, AnyValue value)
    {
        ...
        {
            if (scalar.TryPickError(out XLError scalarError))
            {
                return scalarError;
            }
        ...
        {
            ScalarValue[,] arrayResult = new ScalarValue[array.Height, array.Width];
            for (int row = 0; row < array.Height; ++row)
            {
                for (int col = 0; col < array.Width; ++col)
                {
                    ctx.ThrowIfCancelled();
                    ScalarValue element = array[row, col];
                    if (element.TryPickError(out XLError arrayError))
                    {
    ...
    private static ScalarValue _Text(CalcContext ctx, ScalarValue value, string format)
    {
        ...
        if (!value.ToNumber(ctx.Culture).TryPickT0(out double number, out _) || value.IsLogical)
        {
            return value.ToText(ctx.Culture).Match<ScalarValue>(static x => x, static x => x);
        }
        ...
        bool isDateFormat = nf.IsDateTimeFormat || nf.IsTimeSpanFormat;
    ...
    private static ScalarValue TextJoin(
        CalcContext ctx,
        string delimiter,
        bool ignoreEmpty,
        List<AnyValue> texts
    )
    {
        bool first = true;
        StringBuilder sb = new();
        foreach (AnyValue textValue in texts)
        {
            // Optimization for large areas, e.g. column ranges
            IEnumerable<ScalarValue> textElements = ignoreEmpty
                ? ctx.GetNonBlankValues(textValue)
                : ctx.GetAllValues(textValue);
            foreach (ScalarValue scalar in textElements)
            {
                ctx.ThrowIfCancelled();
                if (!scalar.ToText(ctx.Culture).TryPickT0(out string? text, out XLError error))
                {
                ...
                if (sb.Length > XlsxSharp.XLHelper.CellTextLimit)
                {
                    return XLError.IncompatibleValue;
                }
            }
    ...
    {
        const char space = ' ';
        ReadOnlySpan<char> span = text.AsSpan().Trim(space);
    ...
    private static AnyValue Value(CalcContext ctx, ScalarValue arg)
    {
        ...
        if (arg.TryPickNumber(out double number))
        {
            return number;
        }

        if (!arg.TryPickText(out string text, out XLError error))
        {
            return error;
        }

        const string percentSign = "%";
        bool isPercent = text.IndexOf(percentSign, StringComparison.Ordinal) >= 0;
        string textWithoutPercent = isPercent ? text.Replace(percentSign, string.Empty) : text;
        if (
            double.TryParse(
                textWithoutPercent,
                NumberStyles.Any,
    ...
    private static ScalarValue NumberValue(
        CalcContext ctx,
        string text,
    ...
    )
    {
        ...
        char decimalSep = decimalSeparator[0];
        char groupSep = groupSeparator[0];
    ...
    private static ScalarValue Dollar(CalcContext ctx, double number, double decimals)
    {
        ...
        double rounded = Math.Round(number / factor, 0, MidpointRounding.AwayFromZero);
        if (rounded != 0)
        {
            rounded *= factor;
        }
