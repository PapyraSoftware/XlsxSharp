...
internal static class DateAndTime
{
    ...
    {
        Func<CalcContext, ScalarValue, ScalarValue> dateValue = DateValue;
        Func<CalcContext, ScalarValue, ScalarValue> timeValue = TimeValue;
        ...
        ce.RegisterFunction(
            ...
            2,
            3,
            AdaptLastOptional(Days360, false),
        ...
        ce.RegisterFunction(
            ...
            1,
            Adapt(IsoWeekNum),
            FunctionFlags.Scalar | FunctionFlags.Future
        ...
        ce.RegisterFunction(
            ...
            0,
            Adapt(Today),
            FunctionFlags.Scalar | FunctionFlags.Volatile
    ...
    private static int BusinessDaysUntil(
        CalcContext ctx,
        int firstDay,
        int lastDay,
        ICollection<int> distinctHolidays
    )
    {
        if (firstDay > lastDay)
        {
        ...
        int workDays = lastDay - firstDay + 1;
        int fullWeekCount = Math.DivRem(workDays, 7, out int remainingDays);

        // find out if there are weekends during the time exceeding the full weeks
        for (int day = lastDay - remainingDays + 1; day <= lastDay; ++day)
        {
            if (IsWeekend(day))
            {
                workDays--;
        ...
        foreach (int holidayDate in distinctHolidays)
        {
    ...
    private static ScalarValue Date(CalcContext ctx, double year, double month, double day)
    {
        ...
        // accordingly.
        double yearAdjustment = Math.Floor((month - 1d) / 12.0);
        year += yearAdjustment;
        ...
        double startOfMonth = new DateTime((int)year, (int)month, 1).ToSerialDateTime();
        double serialDate = startOfMonth + day - 1;
    ...
    private static ScalarValue DateDif(
        CalcContext ctx,
    ...
    {
        if (!TryGetDate(ctx, startDateTime, out int startSerialDate))
        {
            return XLError.NumberInvalid;
        }

        if (!TryGetDate(ctx, endDateTime, out int endSerialDate))
        {
            return XLError.NumberInvalid;
        }
        ...
        DateParts startDate = DateParts.From(ctx, startSerialDate);
        DateParts endDate = DateParts.From(ctx, endSerialDate);
        ...
        if (unit == "M")
        {
            ...
            bool isLastMonthComplete = endDate.Day >= startDate.Day;
            return (endDate.Year - startDate.Year) * 12
                + endDate.Month
                - startDate.Month
                - (isLastMonthComplete ? 0 : 1);
        }
        ...
        if (unit == "MD")
        {
            ...
            DateParts adjacentStartDate = startDate with
            {
        ...
        if (unit == "YD")
        {
            bool endFollowsStart =
                endDate.Month > startDate.Month
                || (endDate.Month == startDate.Month && endDate.Day >= startDate.Day);
            int newEndYear = startDate.Year + (endFollowsStart ? 0 : 1);
            DateParts newEndDate = endDate with { Year = newEndYear };
            int daysDiff = newEndDate.SerialDate - startSerialDate;

            // If start date is in 1900 jan/feb, there are sometimes errors. I couldn't decipher actual logic,
            // only condition when it happens. Based on the Excel vs XlsxSharp comparisons, it seems to work.
            if (
                startSerialDate <= 60
                && endDate is { Year: > 1900, Month: 3 }
    ...
    {
        if (!value.TryPickText(out string text, out XLError error))
        {
            return error;
        }

        if (!ScalarValue.ToSerialDateTime(text, ctx.Culture, out double serialDateTime))
        {
    ...
    {
        if (!TryGetDate(ctx, serialDateTime, out int serialDate))
        {
            return XLError.NumberInvalid;
        }
    ...
    {
        if (!TryGetDate(ctx, startSerialDate, out int startDate))
        {
            return XLError.NumberInvalid;
        }

        if (!TryGetDate(ctx, endSerialDate, out int endDate))
        {
            return XLError.NumberInvalid;
        }
    ...
    private static ScalarValue Days360(
        CalcContext ctx,
        double startDateTime,
        double endDateTime,
        bool isEuropean
    ...
    {
        if (!TryGetDate(ctx, startDateTime, out int startDate))
        {
            return XLError.NumberInvalid;
        }

        if (!TryGetDate(ctx, endDateTime, out int endDate))
        {
            return XLError.NumberInvalid;
        }
    ...
    {
        DateParts startDate = DateParts.From(ctx, startSerialDate);
        (int startYear, int startMonth, int startDay) = startDate;
        (int endYear, int endMonth, int endDay) = DateParts.From(ctx, endSerialDate);
    ...
    {
        if (!TryGetDate(ctx, startSerialDate, out int startSerial))
        {
            return XLError.NumberInvalid;
        }

        if (!TryGetMonthsOffset(monthOffset, out int months))
        {
            return XLError.NumberInvalid;
        }

        DateParts startDate = DateParts.From(ctx, startSerial);
        DateParts endDate = startDate.AddMonths(months);
        return endDate.ToSerialDate().Match<ScalarValue>(d => d, e => e);
    }
    ...
    private static ScalarValue Eomonth(CalcContext ctx, double startSerialDate, double monthOffset)
    {
        ...
        DateParts startDate = DateParts.From(ctx, startSerial);
        DateParts endDate = startDate.AddMonths(months);
        DateParts endOfMonth = endDate.EndOfMonth();
    ...
    private static ScalarValue IsoWeekNum(CalcContext ctx, double serialDateTime)
    {
        ...
        DateParts date = DateParts.From(ctx, serialDate);

        // Normalized to Monday = 1, Sunday = 7
        int dayOfWeek = ((int)date.DayOfWeek + 6) % 7 + 1;
        int week = (10 + date.DayOfYear - dayOfWeek) / 7;
    ...
    private static ScalarValue NetWorkDays(
        CalcContext ctx,
        ScalarValue startDate,
        ScalarValue endDate,
        AnyValue holidays
    )
    {
        if (!TryGetDate(ctx, startDate, out int startSerialDate, out XLError startDateError))
        {
            return startDateError;
        }

        if (!TryGetDate(ctx, endDate, out int endSerialDate, out XLError endDateError))
        {
            return endDateError;
        }

        // Use set to skip duplicate values
        HashSet<int> allHolidays = [];
        foreach (ScalarValue holidayValue in ctx.GetNonBlankValues(holidays))
        {
            ctx.ThrowIfCancelled();
            if (!TryGetDate(ctx, holidayValue, out int holidayDate, out XLError error))
            {
                return error;
            }
    ...
    {
        if (!TryGetComponent(hour, out int hourFloored))
        {
            return XLError.NumberInvalid;
        }

        if (!TryGetComponent(minute, out int minuteFloored))
        {
            return XLError.NumberInvalid;
        }

        if (!TryGetComponent(second, out int secondFloored))
        {
            return XLError.NumberInvalid;
        }

        double serialDate = new TimeSpan(
            hourFloored,
            minuteFloored,
            secondFloored
        ).ToSerialDateTime();
    ...
    {
        if (!TryGetDate(ctx, date, out int serialDate, out XLError dateError, acceptLogical: true))
        {
            return dateError;
        }
        ...
        if (!flag.IsBlank)
        {
            ...
            if (!flag.ToNumber(ctx.Culture).TryPickT0(out flagValue, out XLError flagError))
            {
                return flagError;
            }
        }

        OneOf<int, XLError> result = Weekday(serialDate, (int)Math.Truncate(flagValue));

        if (!result.TryPickT0(out int weekday, out XLError weekdayError))
        {
            return weekdayError;
        }
    ...
    private static OneOf<int, XLError> Weekday(int serialDate, int startFlag)
    {
        ...
        int numberOffset = startFlag == 3 ? 0 : 1;
        ...
        int weekday = WeekdayCalc(serialDate, weekStartOffset.Value, numberOffset);
        return weekday;
    }
    ...
    private static ScalarValue WeekNum(
        CalcContext ctx,
        double serialDateTime,
        double weekStartFlag = 1
    )
    {
        if (!TryGetDate(ctx, serialDateTime, out int serialDate))
        {
        ...
        int flag = (int)weekStartFlag;
        DayOfWeek firstDayOfWeek = flag switch
        {
            1 => DayOfWeek.Sunday,
            2 => DayOfWeek.Monday,
            11 => DayOfWeek.Monday,
        ...
        DateParts date = DateParts.From(ctx, serialDate);
        int startOfYearDate = (int)(new DateTime(date.Year, 1, 1).ToSerialDateTime());
        DayOfWeek startOfYearDayOfWeek = (DayOfWeek)((startOfYearDate + 6) % 7);
        int startOfWeekAdjust = firstDayOfWeek - startOfYearDayOfWeek;
        ...
        int firstWeekStartDate = startOfYearDate + startOfWeekAdjust;
        int weekNum = (serialDate - firstWeekStartDate) / 7;
        return weekNum + 1;
    }

    private static ScalarValue Workday(
        CalcContext ctx,
        ScalarValue startDateScalar,
        ScalarValue dayOffsetValue,
        AnyValue holidays
    )
    {
        if (!TryGetDate(ctx, startDateScalar, out int startDate, out XLError startDateError))
        {
            return startDateError;
        }
        ...
        if (
            !dayOffsetValue
                .ToNumber(ctx.Culture)
                .TryPickT0(out double dayOffsetDouble, out XLError dayOffsetError)
        )
        {
            return dayOffsetError;
        }

        int dayOffset = (int)Math.Truncate(dayOffsetDouble);
        ...
        int oneDay = dayOffset > 0 ? 1 : -1; // One day in a specified direction

        if (!GetHolidays(cmp).TryPickT0(out List<int>? orderedHolidays, out XLError holidaysError))
        {
        ...
        int lastDateSoFar = startDate;

        // Number of workdays that have already been processed from startDate up to
        // the lastDateSoFar (inclusive).
        int workdaysSoFar = 0;
        bool startIsHoliday = orderedHolidays.Count > 0 && orderedHolidays[0] == startDate;
        for (int i = startIsHoliday ? 1 : 0; i < orderedHolidays.Count; ++i)
        {
            ctx.ThrowIfCancelled();
            int holidayDate = orderedHolidays[i];
            ...
            int segmentWorkdays =
                lastDateSoFar + oneDay != holidayDate
                    ? BusinessDaysUntil(
                        ctx,
                        lastDateSoFar + oneDay,
                        holidayDate,
        ...
        int remainingWorkdays = dayOffset - workdaysSoFar;
        int weekCount = Math.DivRem(remainingWorkdays, 5, out int remaining);
        ...
        int workday = lastDateSoFar + weekCount * 7;
        while (remaining != 0)
        {
            do
            {
        ...
        OneOf<List<int>, XLError> GetHolidays(IComparer<int> comparer)
        {
            ...
            List<int> sortedHolidays = [.. distinctHolidays];
    ...
    private static ScalarValue YearFrac(
        CalcContext ctx,
        double startDateTime,
        double endDateTime,
        double basis = 0
    )
    {
        ...
        int option = checked((int)Math.Truncate(basis));
        double yearFrac = option switch
        {
        ...
        {
            int startYear = DateParts.From(ctx, startDate).Year;
            int endYear = DateParts.From(ctx, endDate).Year;
            int totalDays = 0;
    ...
    )
    {
        ...
        if (!value.ToNumber(ctx.Culture).TryPickT0(out double serialDateTime, out error))
        {
            serialDate = default;
            return false;
        }
    ...
    private static ScalarValue GetTimeComponent(
        CalcContext ctx,
        double serialTime,
        Func<DateTime, int> component
    )
    {
    ...
    private readonly record struct DateParts(int Year, int Month, int Day)
    {
        ...
        internal int SerialDate =>
            (int)(new DateTime(this.Year, this.Month, 1).ToSerialDateTime()) + this.Day - 1;

        public DayOfWeek DayOfWeek => (DayOfWeek)((WeekdayCalc(this.SerialDate) + 7 - 1) % 7);
        ...
        public int DayOfYear
        {
            get
            {
                DateParts startOfYear = new(this.Year, 1, 1);
                return this.SerialDate - startOfYear.SerialDate + 1;
            }
        }
        ...
        internal static DateParts From(CalcContext ctx, int serialDate)
        {
            ...
            {
                DateTime date1904 = DateTime.FromOADate(serialDate + 1462);
        ...
        internal bool IsLastDayOfMonth()
        {
            ...
            if (this.Year == 1900 && this.Month == 2 && this.Day == 28)
            {
                return false;
            }
        ...
        {
            int shiftedMonth = this.Month + months - 1;
            int adjustYear = (int)Math.Floor(shiftedMonth / 12.0);
            int year = this.Year + adjustYear;
            int month = shiftedMonth - adjustYear * 12 + 1;
            int day = Math.Min(this.Day, DaysInMonth(year, month)); // Uses real Feb28
