...
internal class ScalarValueComparer : IComparer<ScalarValue>
{
    ...
    {
        int xTypeOrder = GetTypeOrder(in x);
        int yTypeOrder = GetTypeOrder(in y);
        int typeCompare = xTypeOrder - yTypeOrder;
        if (typeCompare != 0)
        {
            return typeCompare;
        }
        ...
        if (x.IsText)
        {
            return this._stringComparer.Compare(x.GetText(), y.GetText());
        }
