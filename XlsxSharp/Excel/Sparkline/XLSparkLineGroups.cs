...
internal class XLSparklineGroups : IXLSparklineGroups, IEnumerable<XLSparklineGroup>
{
    ...
    {
        XLSparklineGroup emptyGroup = new(this.Worksheet);
        this.Add(emptyGroup);
        return emptyGroup;
    }
    ...
    {
        if (sparklineGroup.Worksheet != this.Worksheet)
        {
        ...
        this._sparklineGroups.Add((XLSparklineGroup)sparklineGroup);
    ...
    public IXLSparklineGroup Add(string locationAddress, string sourceDataAddress)
    {
        ...
        ArgumentNullException.ThrowIfNull(sourceDataAddress);

        return this.Add(new XLSparklineGroup(this.Worksheet, locationAddress, sourceDataAddress));
    }
    ...
    public IXLSparklineGroup Add(IXLCell location, IXLRange sourceData)
    {
        ...
        return this.Add(new XLSparklineGroup(location, sourceData));
    }

    public IXLSparklineGroup Add(IXLRange locationRange, IXLRange sourceDataRange)
    {
        ArgumentNullException.ThrowIfNull(locationRange);

        ArgumentNullException.ThrowIfNull(sourceDataRange);

        return this.Add(new XLSparklineGroup(locationRange, sourceDataRange));
    }
    ...
    public IXLSparklineGroup AddCopy(
        IXLSparklineGroup sparklineGroupToCopy,
        IXLWorksheet targetWorksheet
    )
    {
        XLSparklineGroup sparklineGroup = new(targetWorksheet, sparklineGroupToCopy);
        this._sparklineGroups.Add(sparklineGroup);
        return sparklineGroup;
    }
    ...
    {
        foreach (XLSparklineGroup slg in this)
        {
    ...
    public IXLSparkline? GetSparkline(IXLCell cell)
    {
        ...
        foreach (XLSparklineGroup sparklineGroup in this._sparklineGroups)
        {
            if (sparklineGroup.TryGetSparkline(location, out XLSparkline? sparkline))
            {
                return sparkline;
            }
        }
    ...
    {
        List<IXLSparkline> sparklinesToRemove =
        [
            .. this
                ._sparklineGroups.SelectMany(g => g)
                .Where(sparkline => range.Contains(sparkline.Location)),
        ];

        sparklinesToRemove.ForEach(this.Remove);
    }
