...
public static partial class XLHelper
{
    ...
    public static string CreateSafeSheetName(string nameProposal, char replaceChar = ' ')
    {
        ...
        string shortenedName = nameProposal.Substring(0, length);
        StringBuilder result = new(shortenedName);
    ...
    public static void ValidateSheetName(String sheetName)
    {
        ...
        if (len < 1 || len > MaxWorksheetNameCharsCount)
        {
            throw new ArgumentException(
                $"sheetName '{sheetName}' is invalid - character count MUST be greater than or equal to 1 and less than or equal to {MaxWorksheetNameCharsCount}"
            );
        }
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            {
                throw new ArgumentException(
                    $"Invalid char ({sheetName[i]}) found at index ({i}) in sheet name '{sheetName}'"
                );
            }
        }
        ...
        {
            throw new ArgumentException(
                $"Invalid sheet name '{sheetName}'. Sheet names must not begin or end with (')."
            );
        }
    }
}
