...
internal class BmpInfoReader : ImageInfoReader
{
    ...
    {
        stream.Position += 14;
        int infoHeaderSize = stream.ReadS32LE();
    ...
    {
        ushort widthPx = stream.ReadU16LE();
        ushort heightPx = stream.ReadU16LE();
    ...
    {
        uint widthPx = stream.ReadU32LE();
        uint heightPx = stream.ReadU32LE();
        stream.Position += 12;
        double dpiX = PixelsPerMeterToDpi(stream.ReadU32LE());
        double dpiY = PixelsPerMeterToDpi(stream.ReadU32LE());
        return new XLPictureInfo(XLPictureFormat.Bmp, widthPx, heightPx, dpiX, dpiY);
    }

    private static double PixelsPerMeterToDpi(uint pixelsPerMeter) => pixelsPerMeter * 2.54d / 100d;
}
