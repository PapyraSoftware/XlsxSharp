...
internal class WebpInfoReader : ImageInfoReader
{
    ...
    protected override XLPictureInfo ReadInfo(Stream stream)
    {
        ...
        uint chunkCode = stream.ReadU32BE();
        ...
        if (chunkCode == ExtendedV8Code)
        {
            ...
            // Skip image features
            stream.Position += 4;
            int width = stream.ReadU24LE() + 1;
            int height = stream.ReadU24LE() + 1;
        ...
        if (chunkCode == LossyVp8Code)
        {
            ...
            int frameTag = stream.ReadU24LE();
            bool isKeyFrame = (frameTag & 1) == 0;
            if (!isKeyFrame)
            {
                throw new ArgumentException("Image is not a key frame.");
            }

            bool showFrameFlag = ((frameTag >> 4) & 0x1) == 1;
            if (!showFrameFlag)
            {
            ...
            int magicBytes = stream.ReadU24BE();
            ...
            ushort widthAndScale = stream.ReadU16LE();
            int width = GetSize(widthAndScale);

            ushort heightAndScale = stream.ReadU16LE();
            int height = GetSize(heightAndScale);
            ...
            {
                int size = sizeAndScale & 0x3FFF;
                int scale = sizeAndScale >> 14;
                return scale switch
                {
                    0 => size,
                    1 => size * 5 / 4,
        ...
        if (chunkCode == LosslessVp8Code)
        {
            ...
            int width = header[0] + ((header[1] & 0x3F) << 8) + 1;
            int height =
                ((header[1] & 0xC0) >> 6) + (header[2] << 2) + ((header[3] & 0xF) << 10) + 1;
