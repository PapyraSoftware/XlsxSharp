...
public readonly struct XLPictureInfo
{
    ...
    public XLPictureInfo(XLPictureFormat format, uint width, uint height, double dpiX, double dpiY)
    {
        ...
        this.Format = format;
        this.SizePx = new Size((int)width, (int)height);
        this.SizePhys = Size.Empty;
        this.DpiX = dpiX;
        this.DpiY = dpiY;
    }

    public XLPictureInfo(XLPictureFormat format, Size sizePx, Size sizePhys)
        : this(format, sizePx, sizePhys, 0, 0) { }

    public XLPictureInfo(
        XLPictureFormat format,
        Size sizePx,
        Size sizePhys,
        double dpiX,
        double dpiY
    ...
    {
        if (this.SizePx.IsEmpty && this.SizePhys.IsEmpty)
        {
            throw new InvalidOperationException("Image doesn't have a size.");
        }

        if (!this.SizePx.IsEmpty)
        {
            return this.SizePx;
        }

        return new Size(
            (int)Math.Ceiling(this.SizePhys.Width / 1000d / 2.54d * dpiX),
            (int)Math.Ceiling(this.SizePhys.Height / 1000d / 2.54d * dpiY)
        );
    }
}
