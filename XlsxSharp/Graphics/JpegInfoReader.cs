...
internal class JpegInfoReader : ImageInfoReader
{
    ...
    {
        if (!stream.TryReadU16BE(out ushort marker) || marker != Marker.SOI)
        {
            return false;
        }

        // Per spec, APP0 should be the first marker, but there are many sloopy encoders
        while (TryGetMarker(stream, out marker) && TryGetLength(stream, out ushort length))
        {
            switch (marker)
            {
                case Marker.APP0:
    ...
    protected override XLPictureInfo ReadInfo(Stream stream)
    {
        ...
        double xDpi = 0,
            yDpi = 0;
        while (TryGetMarker(stream, out ushort marker) && TryGetLength(stream, out ushort length))
        {
            ...
            if (marker == Marker.APP0)
            {
                ...
                byte units = stream.ReadU8();
                ushort xDensity = stream.ReadU16BE();
                ushort yDensity = stream.ReadU16BE();
            ...
            else if (Marker.SOFx.Contains(marker))
            {
                ...
                ushort height = stream.ReadU16BE();
                ushort width = stream.ReadU16BE();
    ...
    private static class Marker
    {
        ...
        public static readonly ushort[] SOFx =
        [
            0xFFC0,
            0xFFC1,
            0xFFC2,
