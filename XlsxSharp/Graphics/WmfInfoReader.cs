...
internal class WmfInfoReader : ImageInfoReader
{
    protected override bool CheckHeader(Stream stream)
    {
        ...
        ushort type = GetU16LE(header, 0);
        ushort headerSize = GetU16LE(header, 2);
        ushort version = GetU16LE(header, 4);
...
#pragma warning restore CA2022
        bool hasPlaceableHeader = header[0] == 0xD7;
        if (hasPlaceableHeader)
        {
            PlaceableHeader placeable = new(header);
            if (placeable.CheckSum == placeable.CalculateCheckSum())
            {
                // Excel ignores inch field of placeable header, but we don't
                int widthHiMetric = ToHiMetric(placeable.BoundingBox.Width, placeable.Inch);
                int heightHiMetric = ToHiMetric(placeable.BoundingBox.Height, placeable.Inch);
                return new XLPictureInfo(
                    XLPictureFormat.Wmf,
                    Size.Empty,
                    new Size(widthHiMetric, heightHiMetric)
                );
            }
        }
        ...
        stream.Position = 18;
        bool complete = false;
        Point viewportOrigin = Point.Empty;
        Point viewportExtent = Point.Empty;
        while (!complete)
        {
            uint recordSizeInWords = stream.ReadU32LE();
            ushort recordType = stream.ReadU16LE();
            switch (recordType)
            {
                case RecordType.Eof:
                    complete = true;
                ...
                case RecordType.SetWindowExtent:
                    short viewportExtentY = stream.ReadS16LE();
                    short viewportExtentX = stream.ReadS16LE();
                    viewportExtent = new Point(viewportExtentX, viewportExtentY);
                    break;

                case RecordType.SetWindowOrigin:
                    short viewportOriginY = stream.ReadS16LE();
                    short viewportOriginX = stream.ReadS16LE();
        ...
        Size physSize = new(
            ToHiMetric(viewportExtent.X - viewportOrigin.X, 96),
            ToHiMetric(viewportExtent.Y - viewportOrigin.Y, 96)
        );
    ...
    private static ushort GetU16LE(Span<byte> s, int i) => (ushort)(s[i + 0] | (s[i + 1] << 8));

    private static short GetS16LE(Span<byte> s, int i) => (short)(s[i + 0] | (s[i + 1] << 8));
    ...
    private class PlaceableHeader
    {
        ...
        {
            this._key = GetU32LE(s, 0);
            this._resourceHandle = GetU16LE(s, 4);
            short bboxLeft = GetS16LE(s, 6);
            short bboxTop = GetS16LE(s, 8);
            short bboxRight = GetS16LE(s, 10);
            short bboxBottom = GetS16LE(s, 12);
            ...
            this.Inch = GetU16LE(s, 14);
            this._reserved = GetU32LE(s, 16);
            this.CheckSum = GetU16LE(s, 20);
        }

        public ushort CalculateCheckSum()
        {
            ushort checkSum = 0;
            checkSum ^= (ushort)((this._key & 0xFFFF0000) >> 16);
            checkSum ^= (ushort)(this._key & 0xFFFF);
            checkSum ^= this._resourceHandle;
            checkSum ^= (ushort)this.BoundingBox.Left;
            checkSum ^= (ushort)this.BoundingBox.Top;
            checkSum ^= (ushort)this.BoundingBox.Right;
            checkSum ^= (ushort)this.BoundingBox.Bottom;
            checkSum ^= this.Inch;
            checkSum ^= (ushort)((this._reserved & 0xFFFF0000) >> 16);
            checkSum ^= (ushort)(this._reserved & 0xFFFF);
            return checkSum;
        }
    }
