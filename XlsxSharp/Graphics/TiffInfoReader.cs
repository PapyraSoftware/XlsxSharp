...
internal class TiffInfoReader : ImageInfoReader
{
    ...
    {
        if (!stream.TryReadU16BE(out ushort byteOrder))
        {
            return false;
        }

        bool usesLittleEndian = byteOrder == ByteOrder.LittleEndian;
        bool usesBigEndian = byteOrder == ByteOrder.BigEndian;
        if (!usesBigEndian && !usesLittleEndian)
        {
            return false;
        }

        TryReadU16 tryReadU16 = usesLittleEndian
            ? StreamExtensions.TryReadU16LE
            : StreamExtensions.TryReadU16BE;
        if (!tryReadU16(stream, out ushort version))
        {
            return false;
        }
    ...
    {
        ushort byteOrder = stream.ReadU16BE();
        bool usesLittleEndian = byteOrder == ByteOrder.LittleEndian;
        Func<Stream, uint> readU32 = usesLittleEndian
            ? StreamExtensions.ReadU32LE
            : StreamExtensions.ReadU32BE;
        Func<Stream, ushort> readU16 = usesLittleEndian
            ? StreamExtensions.ReadU16LE
            : StreamExtensions.ReadU16BE;

        stream.Position += 2; // skip version
        uint ifdOffset = readU32(stream);
        stream.Position = ifdOffset;
        ushort entriesCount = readU16(stream);
        uint width = 0,
            height = 0,
            resolutionUnit = 2;
        ...
        {
            ushort entryTag = readU16(stream);
            ushort entryType = readU16(stream);
            stream.Position += 4; // entryCount
            long nextEntryStart = stream.Position + 4;
            switch (entryTag)
            {
                case Tag.ImageWidth:
        ...
        double dpiX = ToDpi(xResolution, resolutionUnit);
        double dpiY = ToDpi(yResolution, resolutionUnit);
        return new XLPictureInfo(XLPictureFormat.Tiff, width, height, dpiX, dpiY);
    }

    private static uint ReadShortOrLone(
        Stream stream,
        ushort entryType,
        Func<Stream, ushort> readU16,
    ...
    private static double ReadRational(Stream stream, Func<Stream, uint> readU32)
    {
        ...
        uint numerator = readU32(stream);
        uint denominator = readU32(stream);
