...
internal static class OpenXmlPartReaderExtensions
{
    ...
    internal static string? GetAttribute(
        this ReadOnlyCollection<OpenXmlAttribute> attributes,
        string name
    )
    {
        ...
        {
            OpenXmlAttribute attr = attributes[i];
    ...
    internal static bool GetBoolAttribute(
        this ReadOnlyCollection<OpenXmlAttribute> attributes,
        string name,
        bool defaultValue
    )
    {
        string? attribute = attributes.GetAttribute(name);
        return ParseBool(attribute, defaultValue);
    }

    internal static int? GetIntAttribute(
        this ReadOnlyCollection<OpenXmlAttribute> attributes,
        string name
    )
    {
    ...
    internal static uint? GetUintAttribute(
        this ReadOnlyCollection<OpenXmlAttribute> attributes,
        string name
    )
    {
    ...
    internal static double? GetDoubleAttribute(
        this ReadOnlyCollection<OpenXmlAttribute> attributes,
        string name,
        string namespaceUri
    )
    {
        string? attribute = attributes.GetAttribute(name, namespaceUri);
        if (!string.IsNullOrEmpty(attribute))
        {
    ...
    /// </summary>
    internal static Point? GetCellRefAttribute(
        this ReadOnlyCollection<OpenXmlAttribute> attributes,
        string name
    )
    {
    ...
    /// </summary>
    internal static Area? GetRefAttribute(
        this ReadOnlyCollection<OpenXmlAttribute> attributes,
        string name
    )
    {
    ...
    private static bool ParseBool(string? input, bool defaultValue)
    {
        ...
        bool isTrue =
            input == "1" || string.Equals("true", input, StringComparison.OrdinalIgnoreCase);
        if (isTrue)
        {
            return true;
        }
        ...
        bool isFalse =
            input == "0" || string.Equals("false", input, StringComparison.OrdinalIgnoreCase);
        if (isFalse)
        {
            return false;
        }
