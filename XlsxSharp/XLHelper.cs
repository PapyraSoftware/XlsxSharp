...
public static partial class XLHelper
{
    ...
    static XLHelper()
    {
        ...
        letterIndexes = new Dictionary<string, int>(
            XLHelper.MaxColumnNumber,
            StringComparer.Create(ParseCulture, true)
        );
        for (int i = 0; i < XLHelper.MaxColumnNumber; i++)
        {
            ...
            {
                letter =
                    letters[(i - 26) / 26 / 26 - 1] + letters[(i / 26 - 1) % 26] + letters[i % 26];
            }
    ...
    public static int GetColumnNumberFromLetter(string columnLetter)
    {
        ...
        throw new ArgumentOutOfRangeException(
            columnLetter + " is not recognized as a column letter"
        );
    }
    ...
    public static bool IsValidColumn(string column)
    {
        ...
        bool isValid = theColumn[0] >= 'A' && theColumn[0] <= 'Z';
        if (length == 1)
        {
            return isValid;
        }
    ...
    )
    {
        ...
        XLTableRows rows = new(ws);
        IXLRangeRows inserted = insertFunc(numberOfRows, false);
    ...
    private static string Evaluator(Match match, int row, string column)
    {
        ...
        {
            string[] split = match.Groups["one"].Value.Split('$');
            if (split.Length == 1)
            {
            ...
            XLAddress a = XLAddress.Create(match.Groups["one"].Value);
            if (split[0].Length == 0)
            {
    ...
    {
        double ticks = Math.Round(totalDays * TimeSpan.TicksPerDay, MidpointRounding.AwayFromZero);
    ...
    internal static bool ValidateName(string objectType, string newName, out string message)
    {
        ...
        char[] allowedFirstCharacters = ['_', '\\'];
        if (!allowedFirstCharacters.Contains(newName[0]) && !char.IsLetter(newName[0]))
        {
            message =
                $"The {objectType} name '{newName}' does not begin with a letter, an underscore or a backslash.";
            return false;
        }
    ...
    internal static double PixelToNoC(int px, int mdw)
    {
        ...
        int pp = 2 * (int)Math.Ceiling(mdw / 4.0) + 1;
    ...
    {
        int mdw = workbook.GraphicEngine.GetMaxDigitWidth(font, workbook.DpiX).RoundToInt();
        int pixelsWidth = WidthToPixels(width, mdw);
        double columnWidth = PixelToNoC(pixelsWidth, mdw);
        return columnWidth;
    }
    ...
    /// </summary>
    internal static double CalculateColumnWidth(
        double charWidth,
        IXLFontBase font,
        XLWorkbook workbook
    )
    {
        ...
        // it causes inconsistency with conversion to other units, but other places in XlsxSharp do = keep for now.
