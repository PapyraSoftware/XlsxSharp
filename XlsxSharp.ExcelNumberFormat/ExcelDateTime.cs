...
internal class ExcelDateTime
{
    ...
    public ExcelDateTime(double numericDate, bool isDate1904)
    {
        ...
        {
            numericDate += 1462.0;
            this.AdjustedDateTime = new DateTime(
                DoubleDateToTicks(numericDate),
        ...
        else
        {
            ...
            if (internalDateTime < Excel1900ZeroethMinDate)
            {
                ...
                this.AdjustedDateTime = internalDateTime.AddDays(2);
            }
            ...
            {
                this.AdjustDaysPost = 0;
                this.AdjustedDateTime = internalDateTime.AddDays(1);
            }
            else if (internalDateTime < Excel1900LeapMaxDate)
            {
                this.AdjustDaysPost = 1;
                this.AdjustedDateTime = internalDateTime;
            }
            else
            {
                this.AdjustDaysPost = 0;
    ...
    public int Year => this.AdjustedDateTime.Year;

    public int Month => this.AdjustedDateTime.Month;

    public int Day => this.AdjustedDateTime.Day + this.AdjustDaysPost;

    public int Hour => this.AdjustedDateTime.Hour;

    public int Minute => this.AdjustedDateTime.Minute;

    public int Second => this.AdjustedDateTime.Second;

    public int Millisecond => this.AdjustedDateTime.Millisecond;

    public DayOfWeek DayOfWeek => this.AdjustedDateTime.DayOfWeek;

    public string ToString(string numberFormat, CultureInfo culture) =>
        this.AdjustedDateTime.ToString(numberFormat, culture);

    public static bool TryConvert(
        object value,
        bool isDate1904,
        CultureInfo culture,
        out ExcelDateTime result
    )
    {
