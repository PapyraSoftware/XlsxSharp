...
namespace XlsxSharp.Examples.Comments;

public class AddingComments : IXLExample
{
    public void Create(string filePath)
    {
        XLWorkbook wb = new() { Author = "Manuel" };
        AddMiscComments(wb);
    ...
    {
        IXLWorksheet ws = wb.Worksheets.Add("Web");
        ws.Cell("A1").GetComment().Style.Web.AlternateText =
            "The alternate text in case you need it.";
    }

    private static void AddSize(XLWorkbook wb)
    {
        IXLWorksheet ws = wb.Worksheets.Add("Size");
        ...
        ws.Cell("A4")
            .GetComment()
            .Style.Size.SetHeight(30) // The height is set in the same units as row.Height
    ...
    {
        IXLWorksheet ws = wb.Worksheets.Add("Protection");

        ws.Cell("A1").GetComment().Style.Protection.SetLocked(false).Protection.SetLockText(false);
    }

    private static void AddProperties(XLWorkbook wb)
    {
        IXLWorksheet ws = wb.Worksheets.Add("Properties");

        ws.Cell("A1").GetComment().Style.Properties.Positioning = XLDrawingAnchor.Absolute;
        ws.Cell("A2").GetComment().Style.Properties.Positioning =
    ...
    {
        IXLWorksheet ws = wb.Worksheets.Add("Margins");

        ws.Cell("A2")
            .GetComment()
            .SetVisible()
            .AddText("Lorem ipsum dolor sit amet, adipiscing elit. ")
            .AddNewLine()
            .AddText("Nunc elementum, sapien a ultrices, commodo nisl. ")
            .AddNewLine()
            .AddText("Consequat erat lectus a nisi. Aliquam facilisis.");

        ws.Cell("A2").GetComment().Style.Margins.SetAll(0.25).Size.SetAutomaticSize();
    }

    private static void AddColorsAndLines(XLWorkbook wb)
    {
        IXLWorksheet ws = wb.Worksheets.Add("Colors and Lines");
        ...
        ws.Cell("A2")
            .GetComment()
            .Style.ColorsAndLines.SetFillColor(XLColor.RichCarmine)
    ...
    {
        IXLWorksheet ws = wb.Worksheets.Add("Alignment");
        ...
        ws.Cell("A3")
            .GetComment()
            .AddText("Default Alignments:")
            .AddNewLine()
            .AddText("Vertical = Top")
            .AddNewLine()
            .AddText("Horizontal = Left")
            .AddNewLine()
        ...
        ws.Cell("A8")
            .GetComment()
            .AddText("Vertical = Bottom")
            .AddNewLine()
            .AddText("Horizontal = Right");
        ws.Cell("A8")
            .GetComment()
            .Style.Alignment.SetVertical(XLDrawingVerticalAlignment.Bottom)
        ...
        ws.Cell("D3")
            .GetComment()
            .Style.Alignment.SetOrientation(XLDrawingTextOrientation.BottomToTop)
            .Alignment.SetAutomaticSize();

        ws.Cell("E3").GetComment().AddText("Orientation = Top to Bottom");
        ws.Cell("E3")
            .GetComment()
            .Style.Alignment.SetOrientation(XLDrawingTextOrientation.TopToBottom)
            .Alignment.SetAutomaticSize();

        ws.Cell("F3").GetComment().AddText("Orientation = Vertical");
        ws.Cell("F3")
            .GetComment()
            .Style.Alignment.SetOrientation(XLDrawingTextOrientation.Vertical)
            .Alignment.SetAutomaticSize();
    ...
    {
        IXLWorksheet ws = wb.Worksheets.Add("Comments");
        ...
        ws.Cell("E9").GetComment().Style.Alignment.Orientation =
            XLDrawingTextOrientation.TopToBottom;
        ws.Cell("E9").GetComment().Style.Size.SetAutomaticSize();

        ws.Cell("F9").GetComment().AddText("Bottom to Top");
        ws.Cell("F9").GetComment().Style.Alignment.Orientation =
        ...
        IXLComment cE3 = ws.Cell("E3").GetComment();
        ...
        IXLComment cE7 = ws.Cell("E7").GetComment();
        ...
        IXLComment cG3 = ws.Cell("G3").GetComment();
    ...
    {
        IXLWorksheet ws = wb.Worksheets.Add("Visibility");
    ...
    {
        IXLWorksheet ws = wb.Worksheets.Add("Position");
        ...
        ws.Cell("A1")
            .GetComment()
            .Position.SetColumn(3) // Starting from the third column
    ...
    {
        IXLWorksheet ws = wb.Worksheets.Add("Signatures");
