...
public class UsingRichText : IXLExample
{
    public void Create(string filePath)
    {
        ...
        IXLCell cell1 = ws.Cell(1, 1).SetValue("The show must go on...");
        ...
        cell1.GetRichText().Substring(4, 4).SetFontColor(XLColor.Red).SetFontName("Courier"); // Set the color and font for the word "show"

        // On the next example we'll start with an empty cell and add the rich text
        IXLCell cell = ws.Cell(3, 1);
        ...
        foreach (IXLRichString richText in cell.GetRichText())
        {
            if (richText.Bold)
            {
