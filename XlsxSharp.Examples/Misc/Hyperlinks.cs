...
public class Hyperlinks : IXLExample
{
    public void Create(string filePath)
    {
        ...
        ws.Cell(ro, 1)
            .SetHyperlink(new XLHyperlink(@"http://www.yahoo.com", "Click to go to Yahoo!"));
        ...
        ws.Cell(ro, 1).GetHyperlink().ExternalAddress = new Uri(
            @"mailto:SantaClaus@NorthPole.com?subject=Presents"
        );
        ...
        ws.Cell(++ro, 1)
            .SetFormulaA1(
                "=HYPERLINK(\"[Hyperlinks.xlsx]Hyperlinks!B2:C4\", \"Link to range through formula\")"
            );

        ws.Cell(++ro, 1)
            .SetFormulaA1(
                "=HYPERLINK(\"[../Test.xlsx]Sheet1!B2:C4\", \"Link to another file through formula\")"
            );

        // List all hyperlinks in a worksheet:
        IXLHyperlinks hyperlinksInWorksheet = ws.Hyperlinks;
