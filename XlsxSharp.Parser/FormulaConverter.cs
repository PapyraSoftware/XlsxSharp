...
public static class FormulaConverter
{
    ...
    private static string Normalize(TransformedSymbol transformedFormula, string originalFormula)
    {
        ...
        string trimmed = originalFormula.TrimEnd();
        int endLength = originalFormula.Length - trimmed.Length;
        ReadOnlySpan<char> trimmedEnd = originalFormula.AsSpan().Slice(trimmed.Length, endLength);
