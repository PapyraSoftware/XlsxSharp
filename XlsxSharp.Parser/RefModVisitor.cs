...
public class RefModVisitor : IAstFactory<TransformedSymbol, TransformedSymbol, ModContext>
{
    ...
    public TransformedSymbol ErrorNode(ModContext ctx, SymbolRange range, ReadOnlySpan<char> error)
    {
        ...
        ReadOnlySpan<char> symbol = ctx.Formula.AsSpan().Slice(range.Start, range.Length);
        bool sheetIsRefError = symbol.StartsWith(REF_ERROR.AsSpan(), StringComparison.OrdinalIgnoreCase);
        ...
        ReadOnlySpan<char> sheet = symbol.Slice(0, symbol.Length - REF_ERROR.Length - 1);
        string? modifiedSheet = this.ModifySheet(ctx, sheet.ToString());
    ...
    {
        ReferenceArea? modifiedReference = this.ModifyRef(ctx, reference);
    ...
    {
        string? modifiedSheet = this.ModifySheet(ctx, sheet);
        ReferenceArea? modifiedReference = this.ModifyRef(ctx, reference);
    ...
    {
        string? modifiedFirstSheet = this.ModifySheet(ctx, firstSheet);
        string? modifiedLastSheet = this.ModifySheet(ctx, lastSheet);
        ReferenceArea? modifiedReference = this.ModifyRef(ctx, reference);
    ...
    {
        ReadOnlySpan<char> modifiedFunction = this.ModifyFunction(ctx, functionName);
    ...
    {
        string? modifiedSheet = this.ModifySheet(ctx, sheetName);
    ...
    public TransformedSymbol CellFunction(ModContext ctx, SymbolRange range, RowCol cell, IReadOnlyList<TransformedSymbol> arguments)
    {
        ...
        ReadOnlySpan<char> functionName = ctx.Formula.AsSpan(range.Start, ctx.Formula.IndexOf('(', range.Start) - range.Start);
        if (functionName.Equals("LOG10".AsSpan(), StringComparison.OrdinalIgnoreCase))
        {
            ReadOnlySpan<char> modifiedName = this.ModifyFunction(ctx, functionName);
            return s_copyVisitor.Function(ctx, range, modifiedName, arguments);
        }

        RowCol? modifiedCell = this.ModifyCellFunction(ctx, cell);
    ...
    {
        string? modifiedTableName = this.ModifyTable(ctx, table);
