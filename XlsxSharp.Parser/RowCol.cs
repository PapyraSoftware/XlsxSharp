...
public readonly struct RowCol : IEquatable<RowCol>
{
    ...
    /// </summary>
    public int ColumnValue => this._columnIndex + 1;
    ...
    /// Position of a row.
    /// </summary>
    public int RowValue => this._rowIndex + 1;

    /// <summary>
    /// Does <c>RowCol</c> use <em>A1</em> semantic?
    /// </summary>
    public bool IsA1 => this.Style == A1;

    /// <summary>
    /// Does <c>RowCol</c> use <em>R1C1</em> semantic?
    /// </summary>
    public bool IsR1C1 => this.Style == R1C1;
    ...
    /// </summary>
    public bool IsRow => this.ColumnType == None;

    /// <summary>
    /// Is RowCol a part (start or end) of column span?
    /// </summary>
    public bool IsColumn => this.RowType == None;
    ...
    public RowCol(ReferenceAxisType rowType, int rowValue, ReferenceAxisType columnType, int columnValue, ReferenceStyle style)
    {
        ...
        this._columnIndex = columnValue - 1;
        this.RowType = rowType;
        this._rowIndex = rowValue - 1;
        this.Style = style;
    }
    ...
    public RowCol ToR1C1(int anchorRow, int anchorCol)
    {
        ...
        int newRowPosition = ConvertAxis(this.RowType, this.RowValue, anchorRow);
        int newColPosition = ConvertAxis(this.ColumnType, this.ColumnValue, anchorCol);

        return new RowCol(this.RowType, newRowPosition, this.ColumnType, newColPosition, R1C1);
    ...
    public RowCol ToA1(int anchorRow, int anchorCol)
    {
        ...
        return new RowCol(this.RowType, newRowPosition, this.ColumnType, newColPosition, A1);
    }

    internal RowCol? ToA1OrError(int anchorRow, int anchorCol)
    {
        (int newRowPosition, int newColPosition) = this.ToA1Positions(anchorRow, anchorCol);
        if (this.RowType == Relative && newRowPosition is < 1 or > MaxRow)
        {
            return null;
        }

        if (this.ColumnType == Relative && newColPosition is < 1 or > MaxCol)
        {
            return null;
        }
    ...
    private (int Row, int Col) ToA1Positions(int anchorRow, int anchorCol)
    {
        ...
        if (this.IsA1)
        {
            return (this.RowValue, this.ColumnValue);
        }
    ...
    internal StringBuilder AppendA1(StringBuilder sb)
    {
        ...
        {
            case Absolute:
                sb.Append('$').Append(this.GetA1Reference());
                break;

            case Relative:
                sb.Append(this.GetA1Reference());
                break;
        ...
        {
            case Absolute:
                sb.Append('$').Append(this.RowValue);
                break;

            case Relative:
                sb.Append(this.RowValue);
                break;
    ...
    internal void AppendR1C1(StringBuilder sb)
    {
        ...
        AppendAxis(sb, 'R', this.RowType, this.RowValue);
        AppendAxis(sb, 'C', this.ColumnType, this.ColumnValue);
    ...
    {
        int columnIndex = this.ColumnValue;
        string column = string.Empty;
        do
        {
            columnIndex -= 1;
            int index = columnIndex % 26;
            columnIndex -= index;
            columnIndex /= 26;
    ...
    {
        return obj is RowCol other && this.Equals(other);
    }
    ...
    public override int GetHashCode()
    {
        unchecked
        {
            int hashCode = (int)this.ColumnType;
            hashCode = (hashCode * 397) ^ this.ColumnValue;
            hashCode = (hashCode * 397) ^ (int)this.RowType;
            hashCode = (hashCode * 397) ^ this.RowValue;
            hashCode = (hashCode * 397) ^ this.IsA1.GetHashCode();
            return hashCode;
        }
    }
}