...
public static class ReferenceParser
{
    ...
    public static bool TryParseA1(string text, out string? sheetName, out ReferenceArea area)
    {
        ...
        sheetName = null;
        List<Token> tokens = RolexLexer.GetTokensA1(text.AsSpan());
    ...
    {
        bool isValid = IsA1Reference(tokens);
        if (!isValid)
        {
            area = default;
            return false;
    ...
    private static bool TryParseSheetA1(List<Token> tokens, string text, out string sheetName, out ReferenceArea area)
    {
        ...
        Token sheetPrefixToken = tokens[0];
        ReadOnlySpan<char> sheetPrefix = text.AsSpan(sheetPrefixToken.StartIndex, sheetPrefixToken.Length);
        ...
        ReadOnlySpan<char> referenceArea = text.AsSpan().Slice(sheetPrefixToken.Length);
    ...
    private static bool IsA1Reference(IReadOnlyList<Token> tokens)
    {
        ...
        //              | A1_SPAN_REFERENCE
        bool isValid = tokens.Count switch
        {
