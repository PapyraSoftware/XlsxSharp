...
public static class XmlTreeReaderExtensions
{
    ...
    {
        string? text = reader.GetOptionalString(attributeName);
    ...
    public static uint? GetOptionalUIntHex(this XmlTreeReader reader, string attributeName)
    {
        ...
        string? hexString = reader.GetOptionalString(attributeName);
        ...
        if (
            hexString.Length != 8
            || !XStringConvert.TryGetHexValue(hexString.AsSpan(), out uint number)
        )
        {
    ...
    {
        uint count = reader.GetOptionalUInt("count") ?? 1;
    ...
    public static TResult GetStringMappedValue<TResult>(
        this XmlTreeReader reader,
        string attributeName,
        IReadOnlyDictionary<string, TResult> map
    )
    {
        string attributeValue = reader.GetString(attributeName);
        if (!map.TryGetValue(attributeValue, out TResult? result))
        {
            throw PartStructureException.InvalidAttributeFormat(
                attributeName,
                attributeValue,
