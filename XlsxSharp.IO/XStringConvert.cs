...
public class XStringConvert
{
    ...
    /// case insensitive, the <c>x</c> marker is not (<a href="https://github.com/XlsxSharp/XlsxSharp/issues/1154">#1154</a>).
    ...
    public static string? Decode(string? text)
    {
        ...
        int prevSliceNextCharIndex = 0;
        ReadOnlySpan<char> textSpan = text.AsSpan();
        for (
            int i = textSpan.IndexOf('_');
            i >= 0 && i < text.Length - 6;
        ...
        )
        {
            if (IsPattern(textSpan, i))
            {
                ...
                uint codepoint = GetHexValue(textSpan.Slice(i + 2, 4));
    ...
    private static uint GetHexValue(ReadOnlySpan<char> text)
    {
        if (text.Length > 8)
        {
            throw new ArgumentException(
                "Length is too long to be a valid hex number.",
                nameof(text)
            );
        }
