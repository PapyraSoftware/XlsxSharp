...
public class SchemeTypeMap
{
    ...
    /// </summary>
    private readonly Dictionary<ParsletName, (string FinalType, string ItemType)> _parsletMap =
        new();
    ...
    {
        this._parsletMap.Add(complexTypeName, (cSharpType, cSharpType));
        return this;
    }

    public SchemeTypeMap AddComplexTypeMapping(
        ParsletName complexTypeName,
        string cSharpType,
        string csItemType
    )
    {
        this._parsletMap.Add(complexTypeName, (cSharpType, csItemType));
        return this;
    }

    public SchemeTypeMap AddSimpleType(SimpleTypeMapping simpleType)
    {
        this._simpleTypeMap.Add(simpleType.Name, simpleType);
        return this;
    }
            ...
            new SimpleTypeMapping
            {
                ...
                MapValue = xmlName =>
                    ...
                    ?? throw new InvalidOperationException(
                        $"The XML value {xmlName} is not mapped to {csTypeName}."
                    ),
            }
        );
    }
    ...
    {
        this._manualParseCall.Add(name, parseCall);
        return this;
    }
    ...
    {
        string simpleTypeName = attribute.Type ?? throw new InvalidOperationException();
        SimpleTypeMapping simpleType = this._simpleTypeMap[simpleTypeName];
        string expressionTemplate = attribute.IsOptional
            ? simpleType.OptionalTemplate
            : simpleType.RequiredTemplate;
    ...
    {
        if (this._parsletMap.TryGetValue(name, out (string FinalType, string ItemType) map))
        {
    ...
    internal bool TryItemGetValue(
        ParsletName parsletName,
        [NotNullWhen(true)] out string? csItemType
    )
    {
        if (this._parsletMap.TryGetValue(parsletName, out (string FinalType, string ItemType) map))
        {
    ...
    {
        if (this._manualParseCall.TryGetValue(name, out string? manualParseCall))
        {
        ...
        {
            throw new InvalidOperationException(
                $"Parslet '{name}' uses a namespace. Specify the parseCall manually through the '{nameof(SchemeTypeMap)}.{nameof(this.AddParseCall)}()' method"
            );
        }
